...
- **Parameters**:
  ...
  - `filters`: Must include at least one of `user_id`, `agent_id`, `run_id` (for example `{"user_id": "u1"}`). Top-level `user_id`, `agent_id`, or `run_id` raise `ValueError: Top-level entity parameters {'user_id'} are not supported in search(). Use filters={'user_id': '...'} instead.`
  - `top_k`: Maximum results (default: 20). `limit` is not a parameter.
  - `threshold`: Minimum similarity score (default: 0.1)
  - `rerank`: Re-rank results (default: False)
...
- **Purpose**: List memories for one entity scope
- **Parameters**:
  - `filters`: Must include at least one of `user_id`, `agent_id`, `run_id`. Top-level entity kwargs raise the same `ValueError` as `search()`, with `get_all` in the message.
  - `top_k`: Maximum memories to return (default: 20). `limit` is not a parameter.
...
- **Parameters**: `query`; `filters` (entity IDs belong here); `top_k`; `rerank`; `threshold`. Top-level `user_id`, `agent_id`, `app_id`, and `run_id` raise `ValueError: Top-level entity parameters {'user_id'} are not supported in search(). Use filters={'user_id': '...'} instead.` There is no `version` or `limit` argument.
...
- **Parameters**: `filters` (entity IDs belong here); `page`; `page_size`. Top-level entity kwargs raise the same `ValueError` as `search()`, with `get_all` in the message. There is no `version`, `limit`, or `top_k` argument.
- **Returns**: Paginated dict `{"count", "next", "previous", "results"}`
...
recent_conversations = memory.get_all(
    filters={
        "user_id": user_id,
        "metadata.timestamp": {"$gte": last_week},
    },
    top_k=10
)
```
