...
- **Graph:** Added Apache AGE graph store support (#4448)
- **LLMs:** Added MiniMax LLM provider (#4431)
- **MCP:** Added Streamable HTTP transport endpoint (#4122)
- **MCP:** Exposed `infer` param in MCP `add_memories` tool (#4517)
- **Integrations:** Added Mem0 plugin for Claude Code and Cursor (#4518)
- **REST API:** Added optional API key authentication to REST API server (#4442)
...
- **MCP:** Fixed operator precedence in `search_memory` filter (#4474)
- **REST API:** Added missing `limit`, `threshold`, `infer`, `memory_type`, and `prompt` params (#4496)
- **REST API:** Added `timestamp` parameter to `update` function (#3799)
