...
<Tabs>
  <Tab title="Python">
    ...
    | `frequency_penalty` | Penalize frequent tokens (-2.0 to 2.0); set inside the `model` dict | Sarvam |
    | `presence_penalty` | Penalize existing tokens (-2.0 to 2.0); set inside the `model` dict | Sarvam |
    | `seed` | Seed for deterministic sampling; set inside the `model` dict | Sarvam |
    | `stop` | Stop sequences (max 4); set inside the `model` dict | Sarvam |
