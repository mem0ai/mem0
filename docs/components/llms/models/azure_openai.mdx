...
As an alternative to using an API key, the Azure Identity credential chain can be used to authenticate with Azure OpenAI. The table below shows the order of precedence for credential application:
...
To enable Role-Based Access Control (RBAC) for Azure OpenAI
