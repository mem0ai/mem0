...
> **Note:** Gemini 2.0 Flash (`gemini-2.0-flash`, `gemini-2.0-flash-001`, `gemini-2.0-flash-lite`, and `gemini-2.0-flash-lite-001`) was [shut down on 1 June 2026](https://ai.google.dev/gemini-api/docs/models/gemini-2.0-flash). Set `model` explicitly to a current ID such as `"gemini-3.5-flash"` (or `"gemini-3.1-flash-lite"` for the lite class). If you omit `model`, the SDK still defaults to the retired `gemini-2.0-flash` string.
