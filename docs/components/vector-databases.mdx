...
In order to use Deeplake as a vector database, supply the appropriate config and supply the database as indicated below.

<CodeGroup>
```python main.py
from embedchain import App

# load deeplake configuration from yaml file
app = App.from_config(yaml_path="config.yaml")
```
...
vectordb:
  ...
  config:
    path: /tmp/deeplake
    collection_name: my_deeplake_index
```
</CodeGroup>
