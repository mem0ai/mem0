...
| `read_timeout` | boto3 client read timeout in seconds (opt-in) | `None` (botocore default) |
| `connect_timeout` | boto3 client connect timeout in seconds (opt-in) | `None` (botocore default) |
| `boto_client_config` | A `botocore.config.Config` or dict of its kwargs (retries, pooling). Scalars above override matching keys. | `None` |
</Tab>
</Tabs>
