...
config_map = {
    "cohere": {"initial_candidates": 100, "top_k": 10},
    "sentence_transformer": {"initial_candidates": 50, "top_k": 10},
    "huggingface": {"initial_candidates": 30, "top_k": 5},
    "llm_reranker": {"initial_candidates": 20, "top_k": 5}
}
```
...
config = {
    "reranker": {
        ...
        "config": {
            ...
            "top_k": 5  # Fewer results for faster processing
        }
    }
}
```
...
def measure_reranker_performance(config, queries, user_id):
    ...
    for query in queries:
        ...
        results = memory.search(query, filters={"user_id": user_id}, rerank=True)
...
async def parallel_search(config, queries, user_id):
    ...
    tasks = [
        memory.search(query, filters={"user_id": user_id}, rerank=True)
        for query in queries
    ]
