...
| `model`     | LLM model to use for scoring                                                                 | `str`  | `"gpt-4o-mini"` |
| `api_key`   | API key for LLM provider                                                                     | `str`  | `None`         |
| `top_k`     | Number of results to return after reranking                                                  | `int`  | `None`         |
| `llm`       | Optional nested LLM config (`{"provider": ..., "config": {...}}`) for provider-specific fields (e.g. `ollama_base_url`) | `dict` | `None`         |
