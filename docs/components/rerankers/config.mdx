...
For any self-hosted or third-party reranker exposing a Cohere/Jina-style `/rerank` endpoint
(e.g. self-deployed `bge-reranker`, Jina, Voyage, SiliconFlow, Together, or a vLLM-hosted reranker).
...
- `RERANKER_BASE_URL` - Base URL for the OpenAI-compatible reranker
- `RERANKER_API_KEY` - API key for the OpenAI-compatible reranker
