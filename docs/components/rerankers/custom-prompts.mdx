...
<Warning>
  The `scoring_prompt` field is **deprecated** and will be removed in a future version. Setting it still works but emits a `DeprecationWarning` at runtime. The field currently sets the system prompt used by the LLM; custom prompts may be exposed through a different API in a future release.
</Warning>
...
for i, prompt in enumerate(prompts):
    ...
    results = memory.search("test query", filters={"user_id": "test_user"}, rerank=True)
