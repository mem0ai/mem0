...
| `embeddingColumnName` | Name of the column the embedding vector is written to | `embedding` |
| `metadataColumnName` | Name of the JSON column the memory payload is written to. Metadata filters are applied against this column. | `metadata` |
</Tab>
</Tabs>
