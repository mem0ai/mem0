...
We have found that the best way to evaluate datasets is with the help of OpenAI's `gpt-4` model. Hence, we require you to set `OPENAI_API_KEY` as an environment variable. If you don't want to set it, you can pass it in the config argument of the respective evaluation class, as shown in the examples below.

We will assume the following dataset for the examples:
...
data = [
    (
        ...
        "As of the information provided, Elon Musk's net worth is $241.6 billion.",
    ),
    (
        ...
        "Elon Musk owns several companies, including Tesla, SpaceX, Neuralink, and The Boring Company.",
    ),
]
...
for d in data:
    dataset.append(EvalData(question=d[0], contexts=d[1], answer=d[2]))
```
...
```python
from embedchain.eval import ContextRelevance
...
```python
from embedchain.eval import AnswerRelevance
