...
client.add(messages, { user_id })
    .then(response => console.log("Memory saved:", response))
    .catch(error => console.error(error));

// Second interaction: Leveraging stored memory
const query = "Suggest restaurants in San Francisco to eat.";
const filters = {
    "AND": [
        {
            "user_id": user_id
...
Mem0 remembers the user's preference for Indian food and their cheese allergy, using this information to provide more relevant and personalized restaurant suggestions in San Francisco.
