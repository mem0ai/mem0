...
| `dbname`               | Name of the database (for pgvector) |
| `user`                 | Username for database connection (for pgvector) |
| `password`             | Password for database connection (for pgvector) |
| `host`                 | Host where the server is running     |
| `port`                 | Port where the server is running     |
| `embeddingModelDims`   | Dimensions of the embedding model     |
| `hnsw`                 | Enable HNSW indexing (for pgvector) |
