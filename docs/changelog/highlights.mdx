...
<Update label="2026-09-03" description="Mem0 plugin architecture">

**Mem0 Plugins: One Memory Experience Across Nine Hosts**

Mem0 integrations now share a consistent memory contract across Claude Code, Cursor, Codex, Kimi Code, Antigravity, OpenClaw, OpenCode, Pi Agent, and DeepSeek Harness.

- **Consistent behavior:** Supported hosts expose relevant recall, capture completed work, isolate project memory, redact credentials, and fail open when memory is unavailable.
- **Native host integration:** Each plugin translates its host's lifecycle events into the shared memory behavior instead of forcing every agent into one runtime.
- **Python plugin family:** Claude Code, Cursor, Codex, Kimi Code, and Antigravity use generated, self-contained bundles built from one Python core and one set of memory skills.
- **TypeScript plugin family:** OpenClaw, OpenCode, Pi Agent, and DeepSeek Harness reuse the same lifecycle safety, formatting, scoping, identity, and telemetry utilities through their native SDKs.
- **Claude Code preserved:** The existing capture, recall, scoping, and `mem0:sidekick` worktree behavior remain intact.
- **Faster host support:** A new Python host needs a descriptor, native manifest, thin event adapter, and contract test. Shared memory policy stays in the core.
- **Current versions:** Claude Code, Cursor, Codex, Kimi Code, and Antigravity are `0.3.0`. Existing npm package versions remain OpenClaw `1.0.16`, OpenCode `0.2.2`, Pi Agent `0.1.5`, and DeepSeek Harness `0.1.1`.

Open the individual setup guide for [Claude Code](/integrations/claude-code), [Cursor](/integrations/cursor), [Codex](/integrations/codex), [Kimi Code](/integrations/kimi), [Antigravity](/integrations/antigravity), [OpenClaw](/integrations/openclaw), [OpenCode](/integrations/opencode), [Pi Agent](/integrations/pi-agent), or [DeepSeek Harness](/integrations/deepseek-plugin).
...
The DeepSeek Harness agent forgets everything between sessions. [`@mem0/deepseek-plugin`](https://www.npmjs.com/package/@mem0/deepseek-plugin) now recalls relevant context automatically, captures completed turns, and provides two explicit tools against the same memory bank you use from other connected agents.
...
- **Automatic recall:** The `system-prompt/assemble` event searches Mem0 with the latest human prompt and adds unseen results to the model context.
- **Completed-turn capture:** The `session/event` listener stores durable human and assistant turns after completion, not intermediate model output.
- **Native Cordis lifecycle:** The plugin waits for the Harness tool and system-prompt services, registers through native extension points, and cleans up automatically when unmounted.
...
- **Config:** `userId` is required, `apiKey` defaults to `$MEM0_API_KEY`, and `autoRecall` and `autoCapture` default to `true`.

See [DeepSeek Harness](/integrations/deepseek-plugin) for setup and [SDK & Tools](/changelog/sdk) for PR links.

<Note>
  Developer preview. The plugin tracks the evolving DeepSeek Harness plugin API.
</Note>
...
- **Local memory server:** Registers the bundled Mem0 server and authenticates with `MEM0_API_KEY`.
...
The v0.2.11 editor plugin release added:
...
The Antigravity v0.1.3 release added:
...
The Pi Agent v0.1.2 release added:

- **Agent memory tool:** Registers `mem0_memory` for scoped search, add, get, delete, and delete-all operations.
- **Slash commands:** Adds `/mem0-remember`, `/mem0-search`, `/mem0-forget`, `/mem0-tour`, `/mem0-scope`, and `/mem0-status`.
...
- **Relevant command results:** Commands provide visible feedback, and search and forget use thresholded, reranked retrieval.
...
The OpenClaw v1.0.13 release added:

- **Skills-based memory architecture:** Triage and recall skills handle extraction, retrieval, and tool guidance.
...
- **Agent-friendly CLI:** Every CLI command supports `--json` for machine-driven setup and diagnostics.
