...
- **Python SDK v2.0.0** — Single-pass additive extraction, hybrid search (semantic + BM25 + entity boost), spaCy-based entity extraction and linking, batch operations, message persistence, Valkey cluster mode
- **TypeScript SDK v3.0.0** — V3 OSS pipeline with entity extraction (720+ lines), message persistence, batch embeddings, camelCase parameters, DeepSeek LLM provider, graph memory removed from OSS (1088 lines)
- **Benchmark Results:** LoCoMo 71.4 → 91.6 (+20), LongMemEval 67.8 → 93.4 (+26), BEAM 1M @ 64.1

**Breaking Changes:** `add()` returns ADD-only events, `search()` defaults changed, deprecated parameters removed (`org_id`, `project_id`, `enable_graph`), `limit` → `top_k`/`topK`, graph memory removed from TS OSS. See [migration guides](https://docs.mem0.ai/migration/oss-v1-to-v2).
