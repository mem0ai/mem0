...
<Update label="2026-09-03" description="mem0-plugin v0.3.0">

**New Features:**
- **Unified plugin core:** Claude Code, Cursor, Codex, Kimi Code, and Antigravity now build self-contained native and Agent Plugins specification bundles from one Python memory runtime and one set of six skills ([#7203](https://github.com/mem0ai/mem0/pull/7203)).
- **TypeScript lifecycle:** OpenClaw, OpenCode, Pi Agent, and DeepSeek Harness now reuse shared lifecycle safety, formatting, identity, project scoping, and telemetry utilities while keeping their existing packages and native host APIs ([#7203](https://github.com/mem0ai/mem0/pull/7203)).
- **DeepSeek automatic memory:** The Harness plugin recalls relevant context during system-prompt assembly and captures completed turns from the durable session event stream ([#7203](https://github.com/mem0ai/mem0/pull/7203)).
- **Kimi Code guide:** Added installation, verification, lifecycle, and troubleshooting documentation for the native Kimi plugin.

**Improvements:**
- **Host adapters:** Native event differences now live in small adapters. Memory policy, redaction, bounds, failure isolation, and telemetry stay in the shared cores.
- **Generated bundles:** Native and portable Python artifacts contain no symlinks and are validated against their manifests during CI.
- **Claude Code compatibility:** The existing capture, recall, scope, and `mem0:sidekick` worktree behavior are preserved. Public plugin names and memory namespaces are unchanged.

**Removed:**
- Removed obsolete plugin-side consolidation commands, skills, configuration, and runtime paths from OpenClaw, OpenCode, and Pi Agent.

<Note>
  Claude Code, Cursor, Codex, Kimi Code, and Antigravity are version `0.3.0`. OpenClaw remains `1.0.16`, OpenCode `0.2.2`, and DeepSeek Harness `0.1.1`. Pi Agent moves to `0.2.0` because its obsolete Dream and pin APIs were removed.
</Note>
...
- **Skill API calls:** Export, stats, pin, health, memory-reviewer, tour, onboard, context-loader, and weekly stats examples now use the correct filter and `top_k` shapes.
...
- **Skill checklists:** Added a progress checklist to `/mem0:onboard`.
...
**Removed:**
- Removed `/mem0:health --fix` and CLI config fallback.
...
- **Project utilities:** Added list-projects, all-project tour, weekly stats, and competing-tools import.
...
**New Features:**
- **Skill suite:** Added 10 skills for export, import, tool import, forget, health, peek, pin, remember, stats, and digest.
...
- **Focused skill set:** Trimmed to 7 skills: `context-loader`, `forget`, `status`, `search`, `scope`, `remember`, and `tour`.

**New Features:**
- **Memory scopes:** Memory operations support `project`, `session`, and `global` scope, with `/mem0-scope` for changing the default.
- **Expanded telemetry:** OpenCode emits the shared `plugin.*` schema for prompt, bash-error, pre-compact, and session-stop events.
...
<Update label="2026-09-03" description="Antigravity plugin v0.3.0">

**Changed:**
- **Claude Code architecture:** Antigravity now ships as a generated, self-contained bundle built from the same Python capture, recall, scoping, MCP, skill, and telemetry sources as Claude Code ([#7203](https://github.com/mem0ai/mem0/pull/7203)).
- **Native lifecycle:** A thin adapter maps Antigravity's supported `PreInvocation`, `PostToolUse`, and `Stop` events into capture and session handling. Recall remains explicit because `PreInvocation` does not include the user's prompt.
- **Focused interface:** The plugin exposes the local `search_memories` tool and six memory skills: search, status, remember, forget, pause, and resume.
- **Compatibility:** The public plugin name and existing memory namespace remain unchanged.
...
<Update label="2026-09-03" description="Kimi plugin v0.3.0">

**Changed:**
- **Claude Code architecture:** Kimi Code now ships as a generated, self-contained bundle built from the shared Python memory runtime and six canonical memory skills ([#7203](https://github.com/mem0ai/mem0/pull/7203)).
- **Native lifecycle:** The adapter maps Kimi's session, prompt, tool, compaction, shutdown, and subagent events into shared capture and recall behavior.
- **Local memory server:** The plugin uses the bundled `search_memories` server instead of the previous remote MCP-only configuration.
- **Sidekick behavior:** Kimi's sidekick receives parent context through Kimi's native subagent lifecycle without claiming Claude Code worktree isolation.
- **Compatibility:** The public plugin name and existing memory namespace remain unchanged.
...
**New Features:**
- **Skills-mode auto-setup:** Onboarding enables triage, recall, and the full tool profile automatically.
...
- **Tool docs in skills:** Memory triage and recall skills now include full tool reference sections.
...
**New Features:**
- **Skills-based memory architecture:** Added the skill loader, memory triage skill, batched extraction, and skill-aware recall.
...
<Update label="2026-09-03" description="Pi Agent plugin v0.2.0">

**Breaking changes:**
- **Focused memory surface:** Removed the obsolete Dream consolidation and pin commands, skills, configuration, exports, and runtime code. Pi Agent now exposes the same six maintained memory workflows documented across the plugin family.

**Improvements:**
- **Shared runtime behavior:** Reuses the common TypeScript lifecycle, formatting, identity, project-scoping, and telemetry utilities while keeping Pi's native extension API and package name.
- **Publishable entry point:** Loads the built `dist/entry.js` artifact so registry installs do not execute TypeScript source directly.
...
- **Slash commands:** Adds `/mem0-remember`, `/mem0-forget`, `/mem0-search`, `/mem0-tour`, `/mem0-scope`, and `/mem0-status`.
...
- **Agent skills:** Bundles `context-loader`, `remember`, `search`, `forget`, `tour`, and `status` skills.
- **Safety:** Confirmation dialogs protect destructive commands, full memory UUIDs stay visible, outputs are truncated, and all tool actions respect `AbortSignal`.
