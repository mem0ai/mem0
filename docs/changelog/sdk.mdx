...
**Bug Fixes:**
- **Vector Stores:** Normalize similarity scores to `[0, 1]` (higher = better) consistently across all backends. Previously some adapters (FAISS, Chroma, Milvus, Redis, Cassandra, S3 Vectors, and others) returned raw distance metrics where lower was better, causing incorrect ranking in multi-store setups ([#5391](https://github.com/mem0ai/mem0/pull/5391))
