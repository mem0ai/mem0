...
```typescript TypeScript
const result = memory.add("I like to drink coffee in the morning and go for a walk.", { user_id: "alice", metadata: { category: "preferences" } });
```
...
```typescript TypeScript
const relatedMemories = memory.search("Should I drink coffee or tea?", { user_id: "alice" });
```
