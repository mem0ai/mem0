...
<Tip>
Embedchain applications are configurable using YAML file or by directly passing the valid YAML string.
</Tip>
...
2. `llm` Section:
    ...
    - `config`:
        ...
        -  `stream` (Boolean): Controls if the response is streamed back to the user (set to false).
        - `number_documents` (Integer): Number of documents to pull from the database as context, defaults to 1
3. `vectordb` Section:
    ...
    <Note>We recommend you to checkout database specific config [here](https://docs.embedchain.ai/components/vector-databases)</Note>
4. `embedder` Section:
