...
- `RemoteLLMApp` allows you to use a self-hosted Remote LLM as a remote inference service. It is a common use case in the enterprise where the retrieval and LLM reasoning are decoupled.
- You can specify the endpoint_url and response_key in the RemoteLLMConfig.
- Optional authentication can be set using authentication_token in the RemoteLLMConfig.
- To use RemoteLLMApp, you need to install extra dependencies with pip install embedchain[opensource]. For now, it would use sentense transformer model `all-MiniLM-L6-v2`.

```
from embedchain import RemoteLLMApp
from embedchain.config import RemoteLLMConfig, ChatConfig

config = RemoteLLMConfig(endpoint_url="http://fakeurl.com/generate", response_key="generated_texts")
app = RemoteLLMApp(config=config)

# Get answer from the LLM model
result = app.get_llm_model_answer("Hello, world!", ChatConfig(max_tokens=1024, temperature=0.1, top_p=0.95))
```
