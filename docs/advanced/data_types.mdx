...
```python
app.add_local('Seek wealth, not money or status. Wealth is having assets that earn while you sleep. Money is how we transfer time and wealth. Status is your place in the social hierarchy.', data_type='text')
```
...
```python
app.add_local(("Question", "Answer"), data_type="qna_pair")
```
