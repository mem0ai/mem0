...
To add any wikipedia page, use the data_type as `wikipedia`. Eg:

```python
app.add("Wikipedia Page Title", data_type="wikipedia")
```

Note: Refer [`wikipedia`](https://wikipedia.readthedocs.io/en/latest/code.html) for errors raised in case there are multiple pages with same title (`DisambiguationError`) or there are no page found for the given title (`PageError`).
