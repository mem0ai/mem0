...
They are split up into remote (`add`) and local (`add_local`) data types.
You can still use the `add` and `add_local` methods interchangably, this separation is not enforced.
But where they stand in this documentation should give you an idea whether you should pass an URL or data directly.

## Remote data types
You can add local files with the remote `add` method by using the `File:` [URI scheme](https://en.wikipedia.org/wiki/File_URI_scheme).
...
To add any doc/docx file, use the data_type as `docx`. Local files work too. Eg:

```python
app.add_local('docx', 'https://example.com/content/file.docx')
app.add_local('docx', 'content/a_local_file.docx')
```
