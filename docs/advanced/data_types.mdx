...
You can load a code repository, from github, locally or any other source, use the data_type as `repo` and a link or local path. Eg:

```python
app.add('repo', "https://github.com/embedchain/embedchain")
```

While .gitignore is respected, it still needs computation to filter the files, for this reason a remote directory will load cleaner in most instances.
