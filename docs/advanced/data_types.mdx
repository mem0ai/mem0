...
To add mails from your gmail inbox, use the data_type as `gmail`.

The second argument is a search query. You can test queries in your gmail web client.
For all mails, use `*` (behaviour with large inboxes has not been tested, use at your own risk).

To use this you need `credentials.json` in your current directory. Follow these steps to get the credentials

1. Go to the [Google Cloud Console](https://console.cloud.google.com/apis/credentials).
2. Create a project if you don#t have one already.
3. Create an `OAuth Consent Screen` in the project. Don't be alarmed when you have to choose `external`.
...
8. Choose the application type `Web application`. As a name you can choose `embedchain`.
