...
| `async_mode` | boolean (default `true`) | Optional | Controls asynchronous processing. **Removed from the Python and TypeScript SDKs** -- the platform now handles all operations asynchronously by default. Only applies when calling the REST API directly. |
| `output_format` | string (default `v1.1`) | Optional | Response format. **Removed from the Python and TypeScript SDKs** -- responses always use the `v1.1` format. Only applies when calling the REST API directly. |
