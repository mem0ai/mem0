...
| `enable_graph` | boolean (default `false`) | Optional | Set to `true` to create entity nodes and relationships for knowledge graph. |
...
Add Memories automatically enrich the knowledge graph on write. Entity nodes and relationships are created for the stored memory. Downstream `get_all` or search calls can then traverse connected entities.
