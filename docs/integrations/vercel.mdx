...
- A scoped `MEM0_API_KEY` is added to your connected Vercel project's environment variables.
...
<Steps>
  ...
  <Step title="Connect a project">
    Connect the Mem0 resource to a Vercel project. Mem0 injects `MEM0_API_KEY` into that project.
  </Step>
  <Step title="Redeploy">
    Redeploy so the variable is available at runtime, or run `vercel env pull` to use it locally.
  </Step>
</Steps>
...
| --- | --- |
| `MEM0_API_KEY` | Your Mem0 API key, scoped to the org and project created for this installation. It is all you need to call Mem0. |
