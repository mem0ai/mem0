...
This installs the plugin with native memory tools, lifecycle hooks, and all `/mem0:` slash commands — registered by the plugin via the `mem0ai` SDK, with no MCP server to configure. Restart OpenCode to activate.

### Option B — Standalone MCP Server

If you only need the memory tools without the plugin's hooks or skills, point OpenCode at Mem0's hosted MCP server directly — use OpenCode's built-in MCP configurator:

```bash
opencode mcp add
```

Or manually add this to your `opencode.json` (project-level or global at `~/.config/opencode/opencode.json`):
