...
|----------------|------|-------------|
| `config` | **Config** | Adds the plugin's own `opencode-skills/` dir to OpenCode's `skills.paths` so it discovers the skills in place (no copying) and exposes the `/mem0-*` skill commands |
