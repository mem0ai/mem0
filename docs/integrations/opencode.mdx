...
1. A Mem0 API key (starts with `m0-`):
   - <a href="https://app.mem0.ai/dashboard/api-keys?utm_source=oss&utm_medium=integration-opencode">Get your API key</a> (free sign-up at <a href="https://app.mem0.ai?utm_source=oss&utm_medium=integration-opencode">app.mem0.ai</a>)
...
| `experimental.chat.messages.transform` | **Messages transform** | Adds the first-prompt memories to the session's first user message |
| `experimental.text.complete` | **Text complete** | Records the assistant's final response |
| `event` | **Session events** | On `session.idle`, sends the conversation to Mem0 after 5 exchanges, 10 messages, or 40,000 characters, otherwise after 5 idle minutes. On `session.deleted`, sends what is left. Subagent sessions are ignored |
| `experimental.session.compacting` | **Compaction** | Sends the pending conversation before OpenCode compacts it |
| `shell.env` | **Shell env** | Exports `MEM0_USER_ID`, `MEM0_APP_ID`, `MEM0_SESSION_ID`, and `MEM0_BRANCH` to all shell executions |

Recall and capture work like the Mem0 Claude Code plugin. Recall runs once per session, on the first prompt, and returns the top 5 memories for this repository. Later prompts do not search automatically. Capture sends each session's redacted prompts and final assistant responses with `infer=true`. Mem0 extracts repository facts under a hashed repository `agent_id` and personal facts under your `user_id`, using the same instructions and categories as the Claude Code plugin. The OpenCode session ID is the `run_id`.
