...
The plugin provides:
1. **Auto-capture**: Works like the Mem0 Claude Code plugin, sending your prompts and Pi's final responses at checkpoints
2. **Semantic recall**: Searches this repository's memories once, on the first prompt of a session; `mem0_memory` supports additional explicit searches
3. **Monorepo-aware scoping**: Derives the project from the git remote, consistent across subdirectories
...
| Auto-capture | Sends prompts and final responses after 5 exchanges, 10 messages, or 40,000 characters, after 5 idle minutes, before compaction, and at session end |
...
|-------|---------|----------|
| `project` | repository agent_id, or user_id + app_id | **Default.** Project-specific knowledge: decisions, architecture, config |
...
The `app_id` comes from your git remote (`owner-repo`), falling back to the repository root's directory name, so all subdirectories within a monorepo share the same memory pool. The `run_id` is derived from Pi's session file path.

Recall and capture work like the Mem0 Claude Code plugin, regardless of `defaultScope`. Recall runs once per session, on the first prompt of 20 characters or more, and adds the top 5 memories for this repository to the system prompt. Capture sends each session's redacted prompts and Pi's final responses with `infer=true`. Mem0 extracts repository facts under a hashed repository `agent_id` and personal facts under your `user_id`, using the same instructions and categories as the Claude Code plugin.
...
- **Wrong project detected**: The plugin derives `app_id` from the git remote (`owner-repo`). Without a remote it uses the repository root's directory name. Run `/mem0-status` to see the detected project
