...
| `searchThreshold` | `number` | `0.3` | Minimum similarity score (0–1) a memory must reach to count as a match for `/mem0-search`, `/mem0-forget`, and `/mem0-pin`, passed to the search API. Raise it to be stricter; lower it if relevant results are missed. |
...
<Info>
  **Search relevance.** `/mem0-search`, `/mem0-forget`, and `/mem0-pin` pass `searchThreshold` to the mem0 search API, with reranking enabled for higher-precision ordering. mem0 ranks search results by similarity with no relevance floor, so without the threshold an unrelated query would still return the closest (weak) memories — and `/mem0-forget` would offer them for deletion. With the threshold, a query with no sufficiently similar memory reports no match. If a query you expect to match returns nothing, lower `searchThreshold`; raise it to be stricter.
</Info>
