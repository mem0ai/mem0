...
| `searchThreshold` | `number` | `0.2` | Minimum similarity score (0–1) a memory must reach to count as a match for `/mem0-search`, `/mem0-forget`, and `/mem0-pin`, applied client-side. Raise it to be stricter; lower it if relevant results are missed. |
...
<Info>
  **Search relevance.** `/mem0-search`, `/mem0-forget`, and `/mem0-pin` use mem0's reranking and filter out weak semantic matches below `searchThreshold` (client-side, on the public score). mem0 ranks search results by similarity with no relevance floor, so without this an unrelated query would still return the closest (weak) memories — and `/mem0-forget` would offer them for deletion. Reranking spreads scores apart so a genuinely irrelevant query falls below the floor and reports no match. `/mem0-search` shows each result's relevance score, and when the floor hides matches it reports the closest hidden score and how to lower the threshold, so results never silently vanish. If a query you expect to match returns nothing, lower `searchThreshold`; raise it to be stricter.
</Info>
