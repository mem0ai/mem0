---
title: Pi Agent
description: "Add persistent memory to Pi Agent with the Mem0 plugin semantic search, auto-capture, dream consolidation, and 14 skills."
---
...
The plugin provides:
1. **Auto-capture** — Extracts durable facts from every conversation automatically
...
4. **Scoped memory** — Project, session, user, or global scope for fine-grained control
5. **14 skills + 13 commands** — Full memory management from slash commands and agent-guided workflows
...
| `defaultScope` | `string` | `"project"` | Default memory scope: `project`, `session`, `user`, or `global` |
...
| 13 slash commands | Full memory management from the command line |
| 14 skills | Guide the agent on how to use each capability |
...
All actions accept an optional `scope` parameter: `project` (default), `session`, `user`, or `global`.
...
|-------|---------|----------|
| `project` | user_id + app_id | **Default.** Project-specific knowledge — decisions, architecture, config |
| `session` | user_id + app_id + run_id | Ephemeral context for the current session only |
| `user` | user_id | Cross-project personal knowledge — preferences, identity, goals |
| `global` | *(none)* | Everything across all users and projects |

The `app_id` (project) is auto-detected from the working directory name. The `run_id` (session) is derived from Pi's session file path.
...
Dream is an automated memory maintenance process that:
- **Merges** near-duplicate memories into a single entry
- **Resolves** contradictions between memories (keeps the latest)
- **Prunes** stale or outdated entries
- **Respects** pinned memories (marked with `[PINNED]` prefix)

### Auto-Dream

When enabled, dreams trigger automatically when all three gates pass:
1. **Time gate** — at least `minHours` since last dream (default: 24h)
2. **Session gate** — at least `minSessions` sessions since last dream (default: 5)
3. **Memory gate** — at least `minMemories` in the current project (default: 20)

### Manual Dream

Run `/mem0-dream` to trigger consolidation immediately, bypassing the gates.

## Example Workflow

```text
# Session 1: Planning a trip
You: I'm planning a trip to Japan in October. I prefer budget airlines
     and staying in traditional ryokans.

# Mem0 auto-captures:
#   - [goals] Planning trip to Japan in October
#   - [preferences] Prefers budget airlines
#   - [preferences] Prefers traditional ryokans for accommodation

# Session 2 (days later): Continuing trip planning
You: What do you remember about my travel plans?

# Pi searches memories, retrieves Japan trip context
# Responds with full context — no re-explaining needed

# Session 3: Different topic entirely
You: /mem0-search budget airlines

# Returns: "Prefers budget airlines" with similarity score
```
...
- **Wrong project detected** — The plugin uses the working directory name as `app_id`. Use `/mem0-switch <name>` to override
