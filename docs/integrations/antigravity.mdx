...
That's it. This installs the MCP server, lifecycle hooks, and all 16 `/mem0:` skills. The MCP config reads `MEM0_API_KEY` from your environment automatically.

### Option B — MCP Only

If you only need the memory tools without hooks or skills:

```bash
mkdir -p ~/.gemini/config/plugins/mem0
```
...
Create `~/.gemini/config/plugins/mem0/mcp_config.json`:
...
<Info icon="check">
  Start a new Antigravity session and ask: *"Search my memories for hello"*. If the `mem0` tools appear and respond, you're all set.
</Info>

## What's Included

| Component | Full Plugin (A) | MCP Only (B) |
|-----------|:-----------------:|:------------:|
