...
| `memory_add` | Explicitly save a fact |
| `memory_get` | Retrieve a memory by ID |
| `memory_list` | List all stored memories for a user |
| `memory_delete` | Delete a memory by ID |
| `memory_update` | Update an existing memory |
| `memory_event_list` | List memory events (adds, updates, deletes) |
| `memory_event_status` | Get status of a specific memory event |

The `memory_search` and `memory_list` tools accept a `scope` parameter (`"session"`, `"long-term"`, or `"all"`) to control which memories are queried. The `memory_add` tool accepts a `longTerm` boolean (default: `true`) to choose where to store.
