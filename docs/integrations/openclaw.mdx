...
`openclaw mem0 init` enables skills mode. While skills mode is active, the skills handle recall and storage and the `autoRecall` and `autoCapture` hooks are not registered.

With skills mode off, `autoRecall` and `autoCapture` (both on by default) follow the Claude Code plugin. Recall runs once per session, on the first prompt of 20 characters or more, and injects up to `topK` memories with reranking off, only the latest version of each memory, and no score threshold. Capture keeps each turn's redacted prompt and final reply, and sends them after 5 exchanges, 10 messages, or 40,000 characters, after 5 idle minutes, before compaction, and when the session ends.
...
- **Session (short-term)**: Memories stored with the current session's `run_id` / `runId`. These are contextual to the ongoing conversation.
...
Automatic recall and automatic capture use the long-term user scope. Skills-mode recall with `strategy: "always"` also searches the current session.
...
| `autoRecall` | `boolean` | `true` | Inject memories once per session, on the first prompt. Not registered in skills mode. |
...
| `searchThreshold` | `number` | `0.3` | Min similarity (0–1) for `memory_search`. Automatic recall uses no threshold. |
...
Auto-capture and auto-recall are **enabled by default** when skills mode is off. When skills mode is configured (the default after `openclaw mem0 init`), the skills handle memory and these hooks are not registered. Set `autoRecall` or `autoCapture` to `false` to disable the corresponding automatic hook.
