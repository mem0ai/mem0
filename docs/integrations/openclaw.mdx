...
Sidekick is exclusive to the [Claude Code plugin](/integrations/claude-code#sidekick-agent). This plugin does not bundle a Sidekick agent. Native OpenClaw subagents can still recall memories from the parent namespace; Mem0 skips automatic capture for those child sessions.
