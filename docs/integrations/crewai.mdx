...
<Note>
  Newer versions of CrewAI removed the `memory_config={"provider": "mem0"}` shortcut that older guides referenced. CrewAI's built-in `Memory` class now only supports its own local storage backends (LanceDB by default), so Mem0 is wired in explicitly through `MemoryClient` instead of through a CrewAI-native provider setting. This keeps memory under your control and works with any CrewAI version.
</Note>
