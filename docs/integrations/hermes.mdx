...
| `--user-id` | User identifier for OSS/server setup; Platform setup prompts for it |
...
Settings live in `$HERMES_HOME/mem0.json` and are written by `hermes memory setup`. API keys live in that profile's `.env`; OSS configuration may also contain database credentials. Setup writes these files atomically with owner-only permissions.
