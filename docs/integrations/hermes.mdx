...
[plugin README](https://github.com/mem0ai/mem0/tree/main/integrations/hermes-plugin-mem0#local-worktree-preview)
...
The standalone plugin owns its redaction and token-aware batching. Full non-empty user and assistant text is preserved after known-secret redaction; oversized messages are split across requests instead of truncated. Raw tool results and the full historical transcript are not captured. Pattern-based redaction cannot recognize every possible secret.
