...
from pydantic_ai import Agent, ModelMessage, UserPromptPart
...
```python
async def mem0_history_processor(messages: List[ModelMessage]) -> List[ModelMessage]:
    """A history processor to inject Mem0 memories into the PydanticAI agent's context."""
    # Assuming user_id is available in a global context or passed through a custom RunContext
    # For simplicity, let's assume a default user_id for this example.
    ...
    if latest_user_message:
        ...
        if relevant_memories_str:
            # Prepend memories as a system message or part of the user prompt
            # Here, we prepend it as a new UserPromptPart for simplicity, 
            # but a SystemPromptPart could also be used depending on desired behavior.
            # Note: PydanticAI's Agent expects a list of ModelMessage objects.
            # We'll add a new message part with the memories.
            messages.insert(0, ModelMessage(parts=[UserPromptPart(content=relevant_memories_str)]))
...
async def chat_with_mem0_pydanticai(user_input: str, user_id: str = "default_user") -> str:
    ...
    result = await agent_with_memory.run(user_input, message_history=[]) # Pass an empty history, as processor handles it
