...
- insert:
    - id: mem0
      ...
      config:
        ...
        userId: "your-user-id"
        # host: "http://localhost:8000"   # optional: self-hosted / local sandbox
```

To run against a self-hosted / local Mem0 instead of the hosted platform, set `config.host` (defaults to `api.mem0.ai`).
...
Anonymous usage is tracked via PostHog to help improve the plugin, and writes are tagged `source="DEEPSEEK_HARNESS"` for attribution. Disable telemetry with `MEM0_TELEMETRY=false`.
