...
|---|---|
| Automatic recall | Searches once per session, with the first human prompt of 20 characters or more, and adds the top 5 results to the model context |
| Automatic capture | Stores completed human and assistant turns every 5 exchanges, after 5 idle minutes, and when the session is disposed |
