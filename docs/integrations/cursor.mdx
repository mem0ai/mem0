...
Sidekick is exclusive to the [Claude Code plugin](/integrations/claude-code#sidekick-agent). This plugin does not bundle a Sidekick agent.
...
<Note>
  Cursor's `beforeSubmitPrompt` hook can allow or block a prompt, but it cannot add context to that prompt. The plugin therefore gives the agent `search_memories` for recall instead of claiming automatic per-prompt injection.
</Note>
