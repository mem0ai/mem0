...
The single add call carries separate extraction instructions for project facts and personal facts. The Platform reads both and classifies each extracted memory into one of two buckets:

- **Shared project memory** (`agent_id`): one namespace per repository, tagged with the directory it was learned in. Stores what the repository is and what it takes to operate in it: conventions, decisions, constraints, commands that work, and the command that failed together with the one that worked afterwards. Everyone who works in the repository reads and extends the same project memory. Project memory never carries a `user_id`, so a teammate's search never mixes in your preferences.
- **Personal memory** (`user_id`): stores what was learned about you: preferred tools, style, habits, and anything you asked to be remembered. Personal memory follows you between repositories and is private to you.

Values that look like credentials are redacted before anything leaves your computer.

<Frame>
  <img src="/images/memory-scoping-venn.svg" alt="Venn diagram: one add call produces shared project memory (agent_id, visible to team) and personal memory (user_id, private to you). Search returns the OR union of both." />
</Frame>

### Plugin sequence

<Frame>
  <img src="/images/plugin-sequence.svg" alt="Sequence diagram showing the full plugin lifecycle: session-start, first-prompt search, capture events (user-prompt, post-tool, sidekick-start, stop), background extraction via detached flush_worker, and on-demand skills and search tool." />
</Frame>
