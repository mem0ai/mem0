...
The search tool searches across sessions and does not require or accept a session ID. The plugin still records `run_id` internally with each memory update. Set the default scope with the `search_scope` setting or `MEM0_CODE_SEARCH_SCOPE` env var.
