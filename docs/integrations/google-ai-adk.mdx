...
async def auto_save_session_to_memory_callback(callback_context) -> None:
    ...
    """
    try:
        # Access invocation_context from callback_context as shown in ADK documentation
        invocation_context = getattr(callback_context, "_invocation_context", None)
        if not invocation_context:
            memory_logger.debug("No invocation context, skipping memory save")
            return
        ...
        if not session:
            memory_logger.debug("No session found, skipping memory save")
            return

        if not memory_service:
            memory_logger.debug("No memory service configured, skipping memory save")
            return
        ...
        if len(events) < 2:  # Need at least user message + agent response
            memory_logger.debug("Session has insufficient events, skipping memory save")
            return

        memory_logger.info("Auto-saving session to memory via callback")
        await memory_service.add_session_to_memory(session)
        memory_logger.info("Successfully saved session to memory via callback")

    except Exception as e:
        memory_logger.exception(f"Error in auto_save_session_to_memory_callback: {e}")
```
