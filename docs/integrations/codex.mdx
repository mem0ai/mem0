...
For the full plugin experience — MCP server plus the Mem0 SDK skill, memory protocol skill, and opt-in lifecycle hooks — sideload the plugin from a local clone.

<Info>
  Do not combine Option B with Option A. The plugin manifest auto-registers the `mem0` MCP server, so having both will create a duplicate registration.
</Info>
...
This points Codex at the repo's `.agents/plugins/marketplace.json`. The bundled file uses `path: "./mem0-plugin"`, resolved relative to the clone root.

**Step 3.** Restart Codex, run `/plugins`, browse the `Mem0 Plugins` marketplace, and install **Mem0**.

**Step 4 (optional) — enable lifecycle hooks.** Run the bundled installer to merge Mem0 hook entries into `~/.codex/hooks.json`:
...
Restart Codex. The installer registers hooks pointing at scripts inside your clone:
...
Re-running the installer is idempotent. To remove: `python3 ~/codex-plugins/mem0-source/mem0-plugin/scripts/install_codex_hooks.py --uninstall`.

<Warning>
  The hooks file stores absolute paths into your clone. If you move the clone directory, re-run the installer from the new location.
</Warning>
...
To update, `git pull` inside your clone and run `codex plugin marketplace upgrade`.
...
When the plugin is sideloaded, the memory protocol skill instructs the agent to search memories at task start, store key learnings (decisions, anti-patterns, conventions, preferences) after significant work, and capture a session summary before losing context.
...
- **Duplicate `mem0` MCP / "tool collision" errors** — You combined Option A with Option B. Remove the `[mcp_servers.mem0]` block from `~/.codex/config.toml`; the sideloaded plugin registers it automatically
- **Hooks not firing** — Confirm `codex_hooks = true` is in `~/.codex/config.toml` under `[features]` and that `~/.codex/hooks.json` contains the Mem0 entries. Re-run the installer if not, then restart Codex
