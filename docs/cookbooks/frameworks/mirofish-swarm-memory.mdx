...
- Per-agent isolation via app_id
...
class MiroFishSimulation:
    ...
    def record_action(self, agent_id: str, agent_name: str,
        ...
        Agent memories use app_id to group by agent (no assistant
    ...
    def recall_agent_memory(self, agent_id: str, query: str) -> dict:
        """Agent recalls memories relevant to its role mid-simulation.

        Searches project-wide by user_id — semantic matching naturally
        surfaces content relevant to the querying agent's context.
        """
        results = self.memory.search(
            query,
            user_id=self.project_id,
    ...
    def agent_search(self, agent_id: str, query: str, limit: int = 10) -> dict:
        """Search for an agent's activities across the simulation."""
        results = self.memory.search(
            query,
            user_id=self.project_id,
            limit=limit
...
| **Per-agent isolation** | Single shared graph in MiroFish | Native `app_id` scoping |
...
`user_id` scopes the simulation project. `app_id` tags individual agent actions at storage time (we use `app_id` instead of `agent_id` since no assistant memories are involved). Searches use `user_id` for project-wide retrieval:
...
# Search across the simulation (project-wide)
...
<Note>
  Use `user_id` for `add()` when storing project-level data (seed documents). Use `app_id` for `add()` when storing agent actions. For `search()` and `get_all()`, query by `user_id` for project-wide results. Always use the message list format `[{"role": "user", "content": "..."}]` for `add()` — it works on both OSS and Cloud.
</Note>
...
| **3. Simulation** | Track per-agent actions | `memory.add(messages, app_id=agent)` |
| **3. Simulation** | Mid-round recall | `memory.search(query, user_id=project)` |
...
| ZepGraphMemoryUpdater | `client.graph.add(type="text")` | `memory.add(messages, app_id=...)` | No batch buffering or retry — implement in your wrapper |
...
2. **Always set `app_id` for agent actions** — Per-agent isolation prevents memory cross-contamination between agents
