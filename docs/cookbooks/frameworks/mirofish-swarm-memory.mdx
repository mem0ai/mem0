...
<Warning>
**Graph memory has been removed from the Mem0 OSS SDKs** (Python v2.0.0, TypeScript v3.0.0). This cookbook uses the legacy OSS graph-memory integration and will not run against the current OSS release. Entity extraction + linking is now built into the v3 extraction pipeline, but the external graph-store hooks and the `relations` field on search results are no longer available in OSS. For full graph features, use the [Mem0 Platform](https://app.mem0.ai). See the [OSS v2→v3 migration guide](/migration/oss-v2-to-v3).
</Warning>
...
```python
results = memory.search("housing reform", filters={"user_id": "my_sim"})
...
memory.search("housing reform", filters={"user_id": "my_sim"})

# Search agent-specific memories (actions stored with run_id)
memory.search("housing reform", filters={"run_id": "mayor_chen"})

# Get all project-level memories + graph relations
memory.get_all(filters={"user_id": "my_sim"})
```
...
| **2. Environment Setup** | Enrich agent personas from graph | `memory.search(query, filters={"user_id": project})` — returns facts + relations |
| **3. Simulation** | Track per-agent actions | `memory.add(messages, run_id=agent)` |
| **3. Simulation** | Mid-round recall | `memory.search(query, filters={"run_id": agent})` |
| **4. Report Generation** | Targeted analysis | `memory.search(query, filters={"user_id": project})` — memories + graph |
| **4. Report Generation** | Full overview | `memory.get_all(user_id=project)` — all memories + all relations |
| **5. Deep Interaction** | Follow-up queries | `memory.search(query, filters={"user_id": project})` |
...
| ZepEntityReader | `client.graph.node.get(uuid)` | `memory.search(entity_name, filters={"user_id": ...})` | Semantic search, not exact ID lookup |
| ZepEntityReader | `client.graph.node.get_entity_edges()` | `memory.search(entity_name, filters={"user_id": ...})` → `relations` | Returns all matching relations, not edges for a specific node |
...
| ZepToolsService | `search_graph(query, scope)` | `memory.search(query, filters={"user_id": ...})` → memories + relations | |
...
| OasisProfileGenerator | `client.graph.search()` | `memory.search(query, filters={"user_id": ...})` | |
