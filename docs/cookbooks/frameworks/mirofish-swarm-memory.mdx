...
- Per-agent isolation via agent_id
...
class MiroFishSimulation:
    ...
    - Per-agent isolation via agent_id (first-class in OSS)
    ...
    def record_action(self, agent_id: str, agent_name: str,
        ...
        Each action is scoped to the agent via agent_id. Graph Memory
        extracts any new entities/relationships from the action content,
        enriching the knowledge graph as the simulation progresses.
        """
    ...
    def recall_agent_memory(self, agent_id: str, query: str) -> dict:
        """Agent recalls its own memories + graph relations mid-simulation.

        Only returns memories scoped to this agent (via agent_id).
        Also returns graph relations connected to the query entities.
        """
        results = self.memory.search(
            query,
            user_id=self.project_id,
    ...
    def agent_search(self, agent_id: str, query: str, limit: int = 10) -> dict:
        """Search within a single agent's memory + graph space."""
        results = self.memory.search(
            query,
            user_id=self.project_id,
            agent_id=agent_id,
...
| **Per-agent isolation** | Single shared graph in MiroFish | Native `agent_id` scoping |
...
In Mem0 OSS, `user_id` and `agent_id` are stored on the same record. You can combine them freely:

```python
# Store: project + agent scoped
memory.add(text, user_id="my_sim", agent_id="mayor_chen")

# Search all agents in a simulation
memory.search("housing reform", user_id="my_sim")

# Search one agent's memories only
memory.search("housing reform", user_id="my_sim", agent_id="mayor_chen")

# Get all memories + graph relations
memory.get_all(user_id="my_sim")
```

<Note>
  In Mem0 OSS, `user_id` and `agent_id` are stored on the same record, so filtering by both works correctly. Graph nodes also inherit `agent_id` scoping — seed documents (ingested without `agent_id`) create project-wide graph nodes, while agent actions create agent-scoped nodes. Use searches without `agent_id` (just `user_id`) for cross-agent graph queries.
</Note>
...
| **3. Simulation** | Track per-agent actions | `memory.add(action, user_id=project, agent_id=agent)` |
| **3. Simulation** | Mid-round recall | `memory.search(query, user_id=project, agent_id=agent)` |
...
| ZepGraphMemoryUpdater | `client.graph.add(type="text")` | `memory.add(text, user_id=..., agent_id=...)` | No batch buffering or retry — implement in your wrapper |
...
2. **Always set `agent_id`** — Per-agent isolation is the core value prop. In OSS, `agent_id` is a first-class field stored on each record
