...
Now add `agent_id` and `run_id` to keep memories separate:
...
print(chef_memories["results"][0]["memory"])
```

**Output:**
```
prefers Kyoto kaiseki dining experiences
```
...
if not results["results"]:
    ...
    print(f"Found {len(broader['results'])} memories for this user:")
    for mem in broader["results"]:
        print(f"  - {mem}")
