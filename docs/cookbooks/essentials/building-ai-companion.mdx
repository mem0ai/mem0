...
def chat(user_input, user_id):
    ...
    memories = mem0_client.search(user_input, user_id=user_id, top_k=5)
...
mem0_client.add(
    ...
    user_id="max",
    metadata={"expires_on": expiration}
)
```

Store `expires_on` in metadata and use the REST API to set `expiration_date` if you need automatic expiration. Ray stops asking about the ankle once the memory expires or is pruned.
  </Tab>
  <Tab title="Open Source">
```python
from datetime import datetime, timedelta
...
<Tabs>
  <Tab title="Platform">
```python
memories = mem0_client.search("training plan", user_id="max", top_k=5)
