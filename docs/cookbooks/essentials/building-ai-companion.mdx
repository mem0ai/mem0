...
<Tabs>
  ...
  <Tab title="Open Source">
    ...
    <Note>**Setup:** `pip install mem0ai qdrant-client openai ollama` · Run Qdrant on `localhost:6333` (`docker run -p 6333:6333 qdrant/qdrant`) · Install [Ollama](https://ollama.com) and pull `llama3.1:latest` + `nomic-embed-text:latest` · use `embedding_model_dims` **768** for that embedder.</Note>
  </Tab>
</Tabs>
