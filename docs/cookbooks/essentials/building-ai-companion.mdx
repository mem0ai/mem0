...
<Tabs>
  <Tab title="Platform">
    Here we use **Mem0 Platform** (`MemoryClient`): managed infrastructure, zero setup.

    <Note>**Setup:** `pip install mem0ai openai` · [Get your Mem0 API key](https://app.mem0.ai)</Note>
  </Tab>
  <Tab title="Open Source">
    ...
    <Note>**Setup:** `pip install mem0ai qdrant-client openai` · Run Qdrant on `localhost:6333` (`docker run -p 6333:6333 qdrant/qdrant`) · Install [Ollama](https://ollama.com) and pull `llama3.1:latest` + `nomic-embed-text:latest` · use `embedding_model_dims` **768** for that embedder.</Note>
  </Tab>
</Tabs>
