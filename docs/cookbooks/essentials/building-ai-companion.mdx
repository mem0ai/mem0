...
<Tabs>
  <Tab title="Platform">
    Here we use **Mem0 Platform** (`MemoryClient`): managed infrastructure, zero setup. You'll need an OpenAI API key for chat and a Mem0 API key for memory.

    <Note>**Setup:** `pip install mem0ai openai` · [Get your Mem0 API key](https://app.mem0.ai) · [Get your OpenAI API key](https://platform.openai.com)</Note>
  </Tab>
  <Tab title="Open Source">
    ...
    <Note>**Setup:** `pip install mem0ai qdrant-client openai ollama` · Qdrant on `localhost:6333` · pull a chat model (e.g. `llama3.1:latest`) and `nomic-embed-text:latest` · use `embedding_model_dims` **768** for that embedder.</Note>
  </Tab>
</Tabs>
...
<Note>**Categories vs Metadata:** You assign category-like labels in `metadata` on each `add`. If you need guaranteed tags for filtering, set those keys yourself.</Note>
