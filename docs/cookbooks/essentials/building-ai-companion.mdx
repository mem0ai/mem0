...
<Tip>
When you pass a `filters` object, include your entity scope inside the same JSON (as shown above). Top-level `user_id`/`agent_id` arguments are ignored once `filters` is provided to the hosted API, which is why earlier versions of this cookbook returned empty results.
</Tip>
