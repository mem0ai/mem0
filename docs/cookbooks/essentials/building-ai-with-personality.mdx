...
With Mem0's `user_id` and `agent_id` parameters, you can build AI agents that:

✅ Maintain consistent personalities across all users (`agent_id`)
✅ Remember individual user data privately (`user_id`)
✅ Develop unique relationships with each user (store agent context in metadata)
✅ Prevent data leakage between users
✅ Update agent behavior in one place

**The fitness coach you built demonstrates:**

- User workout tracking (user_id)
- Shared coaching personality (agent_id)
- Personalized relationships (user_id with agent_id in metadata)

Most apps only need `user_id`. Add `agent_id` when your AI needs a consistent personality that evolves independently from user data.

---

## Next Steps

Now that you understand memory scoping, explore:

- **Memory expiration** - Automatically delete old workout logs while keeping user profiles
- **Quality control** - Filter what gets stored (e.g., only validated exercises)
- **Graph memory** - Connect related exercises and muscle groups

We're excited to see what you build with Mem0's flexible memory system!
