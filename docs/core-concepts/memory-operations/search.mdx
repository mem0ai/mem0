...
Each result includes `score_details` with the semantic score, normalized BM25 score, entity boost, the blend `weights`, the final score, and the threshold used for filtering. The field is omitted unless `explain` is enabled, so existing response shapes stay unchanged.

The final score is a fixed weighted blend, `0.6 * semantic + 0.3 * bm25 + 0.1 * entity`. The weights are constant and sum to 1, so a score is always in `[0, 1]` and means the same thing from one query to the next. A signal a memory does not have contributes 0 rather than rescaling the others.
