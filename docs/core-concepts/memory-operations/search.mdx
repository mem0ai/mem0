...
| **org_id scoping** | Via `org_id` in client init or filters | As parameter `org_id="acme"` in `search()` for org-scoped multi-tenant queries |
...
# Org-scoped search: personal + org-wide memories in one call
memories = m.search(
    "quarterly targets",
    org_id="acme",
    filters={
...
**Filter by Organization (OSS, multi-tenant):**
```python
# Search all memories within an org (no user_id constraint)
m.search("company policies", org_id="acme")

# Search personal + shared memories in one call
m.search("project updates", org_id="acme", filters={
    "OR": [
        {"user_id": "alice"},
        {"source": "company_knowledge"}
    ]
...
- **Multi-tenant org search (OSS)**: Use `org_id="acme"` to search across multiple user namespaces within an organization. When `org_id` is provided, `user_id` is not injected as a hard filter — use explicit `OR` filters to control which user scopes to include
