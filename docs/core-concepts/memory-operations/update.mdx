...
<Note>
  In the Python OSS SDK, `text` is optional — pass only `metadata` and/or `expiration_date` to update those while keeping the existing content (at least one of the three must be provided). The JavaScript OSS SDK takes the content as a required positional argument: `update(memoryId, text, metadata?, expirationDate?)`.
</Note>

<Note>
  In the Python OSS SDK, `data=` is a deprecated alias for `text` — it still works but logs a warning; prefer `text`.
</Note>
