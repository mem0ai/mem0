...
<Note>
  `data` is a deprecated alias for `text` in both OSS SDKs (`data=` in Python, `{ data: ... }` in JavaScript). It still works but logs a warning; prefer `text`. The JavaScript positional form `update(memoryId, text, metadata?, expirationDate?)` also still works.
</Note>
