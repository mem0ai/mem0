...
| Category | Previous | New (top200) | New (top50) | Delta (top200) |
|---|---|---|---|---|
...
<Warning>
BEAM reports scores as a **pass rate (0-1 scale)**, not percentage accuracy (0-100) like LoCoMo and LongMemEval. A BEAM score of 0.64 means 64% of questions were answered correctly at that scale. The different scale reflects BEAM's stricter evaluation methodology for large-context memory tasks.
</Warning>
...
LoCoMo and LongMemEval scores are accuracy percentages (0-100). BEAM scores are pass rates (0-1 scale) -- 0.64 = 64% pass rate. All benchmarks run on `gpt-oss-120B`. Scores carry a ±1 point confidence interval due to judge inconsistency.
...
| `--answerer-model` | `gpt-5` | LLM for generating answers from retrieved memories |
| `--judge-model` | `gpt-5` | LLM for judging answer correctness |
...
<AccordionGroup>
  <Accordion title="What judge model is used for evaluation?">
    The default judge model is `gpt-5`. You can override it with `--judge-model` and `--judge-provider` flags. Scores carry a ±1 point confidence interval due to judge inconsistency.
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Platform Migration" icon="arrow-right" href="/migration/platform-v1-to-v2">
