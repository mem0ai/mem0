...
- **A production-representative model, by choice**: All benchmarks use a smaller, production-grade model as the extraction model — roughly 2.6x faster and 18x cheaper than frontier-model stacks
...
- **Compare at equal constraints.** Always compare systems using the same `top_k`, the same model, and the same latency budget. A system scoring 95% with a frontier model at top200 is not comparable to one scoring 90% with a smaller production-grade model at top50.
