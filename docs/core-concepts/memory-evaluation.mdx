...
Mem0's accuracy and token efficiency come from two design choices: **ADD-only extraction** that distills conversations into facts without overwriting history (preserving temporal context), and **multi-signal retrieval** (semantic, keyword, and entity) that surfaces only what matters per query. That is what keeps retrieval averaging under ~7,000 tokens while staying accurate. For the full pipeline, see [How Mem0 Works](/core-concepts/how-it-works).
...
The full evaluation framework is open-source in the [memory-benchmarks repository](https://github.com/mem0ai/memory-benchmarks), so you can reproduce every number here. It supports both Mem0 Cloud and self-hosted OSS backends.
...
Run any benchmark against Mem0 Cloud (set `MEM0_API_KEY`) or a self-hosted server (add `--backend oss`):

```bash
# LoCoMo (~300 questions, fastest)
python -m benchmarks.locomo.run --project-name my-eval --backend cloud --mem0-api-key $MEM0_API_KEY

# LongMemEval (500 questions)
python -m benchmarks.longmemeval.run --project-name my-eval --backend cloud --mem0-api-key $MEM0_API_KEY --all-questions

# BEAM (1M / 10M token scale)
python -m benchmarks.beam.run --project-name my-eval --backend cloud --mem0-api-key $MEM0_API_KEY --chat-sizes 1M
```

For the full CLI options, self-hosted Docker setup, custom model configs, and the results UI, see the [repository README](https://github.com/mem0ai/memory-benchmarks).
