...
Mem0's advanced attribution capabilities now allow you to create multi-user collaborative systems by attaching an **`actor_id`** to each memory. By setting the users's name in `message["name"]`, you can build powerful team collaboration tools where contributions are properly attributed to their authors.
...
| **Multi-User Chat** | Multiple users conversing with a single assistant | `run_id` for shared space, each user with unique `actor_id` |
...
The agent architecture remains the same as before, with a focus on enhancing the implementation to meet the feedback requirements.

## Implementation

Below is a complete implementation of a collaborative task agent that demonstrates how to build team-oriented applications with Mem0. The agent has been enhanced based on feedback to provide more relevant and organized information.
