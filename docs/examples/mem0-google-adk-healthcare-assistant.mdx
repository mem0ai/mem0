...
1. **Memory Storage**: When Sarah mentions her headaches and penicillin allergy, the agent stores this information in Mem0 using the `save_patient_info` tool.

2. **Contextual Retrieval**: When Sarah asks about headache causes, the agent uses the `retrieve_patient_info` tool to recall her specific situation.

3. **Memory Application**: When discussing medications, the agent remembers Sarah's penicillin allergy without her needing to repeat it, providing safer and more personalized advice.
