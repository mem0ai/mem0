...
To use GoogleDriveFolderLoader you must install the extra dependencies with `pip install --upgrade embedchain[googledrive]`.

The data_type must be `google_drive_folder`. Otherwise, it will be considered a regular web page.
...
app.add(url, data_type="google_drive_folder")
```