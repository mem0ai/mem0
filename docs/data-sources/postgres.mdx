...
postgres_loader.setup_loader(config=config)
```
...
app.add("SELECT * FROM table_name;", data_type='postgres', loader_class=postgres_loader)
...
app.add("SELECT * FROM table_name;", data_type='postgres', loader_class=postgres_loader, chunker_class=postgres_chunker)
```