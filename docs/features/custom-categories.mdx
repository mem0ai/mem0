...
> Note: The more detailed the description of categories is, the better output the user will receive. When custom categories are provided, they will completely replace the default categories, so make sure to include all categories you want to use.

> Note: We will soon release a feature that allows users to set custom categories in `project`. The final categories will be a combination of the default and user-defined custom categories.
