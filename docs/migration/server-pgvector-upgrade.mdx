...
The Mem0 API container runs `alembic upgrade head` on startup. Restart it so it picks up the restored database:
...
curl -s http://localhost:8888/v1/memories/ \
  -H "Authorization: Bearer <your-api-key>" | head
```
