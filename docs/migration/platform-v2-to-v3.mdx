...
| **Scoring method** | Raw cosine similarity | Combined multi-signal score (semantic + BM25 + entity) |
...
### Retrieval is now multi-signal

Search fuses three signals — semantic similarity, BM25 keyword match, and entity linking — into a single combined `score` per result. Results preserve the same shape as before:
...
The `score` field is a normalized combined score (0-1). Per-signal component scores are available internally for debugging via the search event (see the [Event API](/api-reference/events)) but are not exposed on the search response itself so the scoring stack remains free to evolve.
...
| List memories (paginated) | `POST /v3/memories/` |
...
# page == {"count": 123, "next": "...", "previous": null, "results": [...]}
```
