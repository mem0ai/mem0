...
1. **Sign up**: Create an account on <a href="https://app.mem0.ai?utm_source=oss&utm_medium=migration-oss-to-platform">Mem0 Platform</a>.
...
<Warning>
  **Critical Change**: `search()` and `get_all()` on current OSS and on Platform both reject top-level `user_id`, `agent_id`, and `run_id`. Put them in `filters`, or the call raises `ValueError: Top-level entity parameters {'user_id'} are not supported in search(). Use filters={'user_id': '...'} instead.`
</Warning>

<Note>
  `search()` takes `top_k`, not `limit`. OSS `get_all()` also takes `top_k`. Platform `get_all()` paginates with `page` and `page_size`.
</Note>

| Method | Open Source | Platform |
| ------ | ----------- | -------- |
| `search()` | `m.search(query, filters={"user_id": "alex"})` | `client.search(query, filters={"user_id": "alex"})` |
| `get_all()` | `m.get_all(filters={"user_id": "alex"}, top_k=20)` | `client.get_all(filters={"user_id": "alex"})` |
...
<AccordionGroup>
  <Accordion title="Search Memories">
    ...
    results = m.search("user's preferences", filters={"user_id": "alex"})

    # Search with multiple entity ids in one filters dict
    results = m.search("meeting notes", filters={"user_id": "alex", "agent_id": "assistant"})
    ```
  ...
  <Accordion title="Get All Memories">
    <CodeGroup>
    ```python Open Source (Old)
    # Get all memories for a user. top_k bounds the list; there is no offset.
    memories = m.get_all(filters={"user_id": "alex"}, top_k=10)
    ```

    ```python Platform (New)
    # Get all memories for a user
    memories = client.get_all(filters={"user_id": "alex"}, page=1, page_size=10)
...
| **Search Method** | `m.search(query, filters={"user_id": "x"})` | `client.search(query, filters={"user_id": "x"})` | Keep entity IDs inside `filters` |
| **Get All Method** | `m.get_all(filters={"user_id": "x"}, top_k=20)` | `client.get_all(filters={"user_id": "x"}, page=1, page_size=50)` | Keep entity IDs inside `filters`. Platform paginates with `page` / `page_size` |
...
- <a href="https://app.mem0.ai?utm_source=oss&utm_medium=migration-oss-to-platform">Platform Dashboard</a> - Monitor usage and manage settings.
