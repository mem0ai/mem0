...
<Info>
The Python `[nlp]` extra installs [spaCy](https://spacy.io/) for richer entity extraction and English keyword lemmatization. Without it, Mem0 still works with regex-based entity extraction and dependency-free keyword tokenization, but statistical NER/noun-chunk coverage and English lemmatization are reduced.
</Info>

<Warning>
**Qdrant users: install `fastembed` to enable BM25 keyword search.** The Qdrant backend uses [fastembed](https://github.com/qdrant/fastembed) to encode sparse (BM25) vectors alongside dense vectors in the same collection. Without it, Qdrant cannot encode sparse BM25 vectors, so the BM25 keyword signal is disabled and the first search call logs a warning. Other vector stores use their native full-text capabilities and do not need `fastembed`.
