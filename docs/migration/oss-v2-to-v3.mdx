...
| Entity ID validation | Accepted any string | Trimmed; empty / whitespace-only rejected | Pass a non-empty identifier without internal spaces |
| `messages` in `add()` | Could be `None` | Required — raises `ValueError` on `None` | Always pass a string, dict, or list of messages |
| `add()` events | Returns `ADD`, `UPDATE`, `DELETE` | Returns `ADD` only | Update code expecting UPDATE/DELETE |
| `score` meaning | Raw cosine similarity | Combined multi-signal score | Score is a fused value (semantic + BM25 + entity); relative ranking is preserved but absolute numbers shift |
...
<Tabs>
  <Tab title="Python OSS">
    ...
    for r in results:
        print(r["score"])   # Combined multi-signal score (semantic + BM25 + entity)
    ```
...
<AccordionGroup>
  ...
  <Accordion title="ValueError: Top-level entity parameters not supported in search() / get_all()">
  ...
  <Accordion title="Score values are different from before">
    The top-level `score` is now a combined multi-signal score (0-1), not raw cosine similarity. It fuses semantic similarity, BM25 keyword match, and entity boost into one value. Relative ranking stays comparable, but absolute numbers shift — retune any hard thresholds in your app against representative queries.

    If you need the raw cosine similarity, run an unboosted vector query directly against your vector store (`vector_store.search(...)`); the Memory class no longer exposes per-signal scores on results.
  </Accordion>
</AccordionGroup>
