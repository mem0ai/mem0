...
|---|---|---|---|
| Search parameter | `search(top_k=10)` | `search(limit=10)` | Rename `top_k` → `limit` |
...
| Graph toggle | `enable_graph=True` in config | Removed | Graph activates automatically when `graph_store` config is present |
...
| Graph toggle | `enableGraph: true` in config | Removed | Graph activates when `graphStore` config is present |
| Default graph config | Neo4j default config applied | No default graph config | Explicitly provide `graphStore` if needed |
| LMStudio provider | Supported | Removed | Migrate to another supported provider |
| DeepSeek provider | Supported | Removed | Migrate to another supported provider |
...
<Tabs>
  <Tab title="Python OSS">
    ...
    # Before
    config = {
        "custom_fact_extraction_prompt": "Focus on user preferences",  # ❌ Renamed
        "custom_update_memory_prompt": "Be concise when updating",      # ❌ Deprecated
        "graph_store": {
            "provider": "neo4j",
            "config": { "url": "...", "username": "...", "password": "..." }
        },
        "enable_graph": True,  # ❌ Removed
    }

    # After
    config = {
        "custom_instructions": "Focus on user preferences",  # ✅ New name
        # custom_update_memory_prompt removed — use custom_instructions
        "graph_store": {
            "provider": "neo4j",
            "config": { "url": "...", "username": "...", "password": "..." }
        },
        # enable_graph removed — graph activates automatically when graph_store is present
    }
    ```
  </Tab>
  <Tab title="TypeScript OSS">
    ...
    // Before
    const config = {
      customPrompt: "Focus on user preferences",  // ❌ Renamed
      enableGraph: true,                            // ❌ Removed
      graphStore: {
        provider: "neo4j",
        config: { url: "...", username: "...", password: "..." }
      }
    ...
    // After
    const config = {
      customInstructions: "Focus on user preferences",  // ✅ New name
      // enableGraph removed — graph activates when graphStore is present
      graphStore: {
        provider: "neo4j",
        config: { url: "...", username: "...", password: "..." }
      }
...
<Tabs>
  ...
  <Tab title="TypeScript OSS">
    ...
    const results = await m.search("what meetings did I attend?", {
      userId: "alice",
      limit: 20          // ❌ Renamed to 'topK'
    });

    // After
    const results = await m.search("what meetings did I attend?", {
      userId: "alice",
      topK: 20            // ✅ Renamed from 'limit'
    });
    ```
  </Tab>
  ...
  <Tab title="TypeScript Client SDK">
    ...
    const client = new MemoryClient({
      ...
      projectId: "proj-1"       // ❌ Removed
    });
    const results = await client.search("query", {
      user_id: "alice",         // ❌ snake_case
      top_k: 20,                // ❌ snake_case
      enable_graph: true        // ❌ Removed
    });
...
<Tabs>
  <Tab title="Python OSS">
    ...
    for item in result["results"]:
        if item["event"] == "ADD":
            ...
            print("Updated:", item["memory"])     # ❌ No longer returned
        elif item["event"] == "DELETE":
            print("Deleted:", item["memory"])      # ❌ No longer returned
  ...
  <Tab title="TypeScript Client SDK">
    ...
    await client.add(messages, {
      user_id: "alice",       // ❌ snake_case
      async_mode: true,       // ❌ Removed
      output_format: "v1.1",  // ❌ Removed
      enable_graph: true      // ❌ Removed
    });
...
<AccordionGroup>
  ...
  <Accordion title="Python OSS — Removed/renamed parameters">
    ...
    **Config:** `enable_graph` → removed (graph activates when `graph_store` config is present)

    **search():** `top_k` → renamed to `limit`
  </Accordion>
  <Accordion title="TypeScript OSS — Removed/renamed parameters">
    **Config:** `customPrompt` → renamed to `customInstructions`

    **Config:** `enableGraph` → removed (graph activates when `graphStore` config is present)

    **search():** `limit` → renamed to `topK`

    **Providers removed:** LMStudio (LLM + Embedder), DeepSeek (LLM)
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Python: 'top_k' is not a valid parameter">
    The `top_k` parameter has been renamed to `limit` in the Python OSS Memory class:
    ```python
    # Before
  ...
  <Accordion title="Graph memory not activating">
    The `enable_graph` / `enableGraph` flag has been removed. Graph memory now activates automatically when a `graph_store` / `graphStore` configuration is present. Make sure your config includes the graph store section:
    ```python
    config = {
        "graph_store": {
            "provider": "neo4j",
