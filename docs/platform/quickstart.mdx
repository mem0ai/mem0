...
<Note>
    Mem0 now supports an contextualized add version (v2). To use it, set `version="v2"` during the add call. The default version is v1, which will be deprecated in the future. We recommend migrating to v2 for new applications. [Learn more](/features/advanced-add).
</Note>
