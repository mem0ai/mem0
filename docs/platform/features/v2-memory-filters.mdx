...
A single bare condition is fine on its own, no wrapper required:
...
But the moment you have more than one condition, a flat object with multiple sibling keys is rejected. Wrap them in `AND` (or `OR`/`NOT`) instead:

```python
# Rejected: multiple sibling keys with no logical wrapper
filters = {
    "user_id": "user_123",
    "categories": {"in": ["finance"]}
}

# Correct: wrap the conditions in AND
filters = {
    "AND": [
        {"user_id": "user_123"},
        {"categories": {"in": ["finance"]}}
...
This wrapper requirement is specific to the hosted Platform API. The self-hosted OSS SDK is more forgiving: sibling top-level keys are implicitly ANDed there, so `{"user_id": "user_123", "categories": {"in": ["finance"]}}` works as-is. See [Enhanced Metadata Filtering](/open-source/features/metadata-filtering) for the OSS grammar.
</Callout>
