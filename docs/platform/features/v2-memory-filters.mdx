...
icon: "filter"
iconType: "solid"
description: Complete guide to filtering memories by user, agent, run, and content criteria.
---

# Memory Filters v2

This guide covers the filtering system for retrieving and searching memories. You can filter by user sessions, agents, applications, content categories, and time ranges.
...
```json
{ "AND": [ { "created_at": { "gt": "2025-01-01T00:00:00Z" } } ] }
```
...
```json
{ "AND": [ { "NOT": { "categories": { "in": ["spam", "test"] } } } ] }
```
