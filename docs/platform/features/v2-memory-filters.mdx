...
<AccordionGroup>
  ...
  <Accordion title="Org-scoped multi-tenant search (OSS)">
    ```python
    # Search personal + org-wide memories in one call
    # When org_id is provided, user_id is not injected as a hard filter
    filters = {
        "OR": [
            {"user_id": "alice"},
    ...
    results = m.search("quarterly targets", org_id="acme", filters=filters)
    ```
  </Accordion>
...
- In OSS, use `org_id` in `search()` to query across multiple `user_id` namespaces within an organization without multiple API calls.
