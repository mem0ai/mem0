...
client = MemoryClient(api_key="sk-platform-...")
```
...
results = client.search("Any dietary flags?", filters=filters)
```

<Info icon="check">
`results["results"][0]["memory"]` should show the shellfish restriction. If the list is empty, verify that all three identifiers match exactly and the session hasn't been archived.
</Info>

**3. Audit everything for an app**
```python
app_scope = {
    "AND": [
        {"app_id": "concierge_portal"},
...
<Info icon="check">
A successful delete returns `{"status": "success"}`. Run the previous `get_all` call again to confirm the session memories were removed.
</Info>
