...
<Note type="warning">
  **Parameter Removed from SDK**

  The `async_mode` parameter has been removed from the Python and TypeScript SDKs. The platform now handles all memory operations asynchronously by default. You no longer need to pass `async_mode` when calling `add()` or other methods through the SDK. If you need synchronous behavior, use the REST API directly with `async_mode: false`.
</Note>

<Note type="warning">
  **Historical Context**

  The `async_mode` parameter previously defaulted to `true` for all memory additions, changing the default API behavior to asynchronous processing. The information below is preserved for reference for users working directly with the REST API.
</Note>
...
# The SDK no longer accepts async_mode; operations are asynchronous by default.
# To force synchronous processing, use the REST API directly (see cURL example).
messages = [
