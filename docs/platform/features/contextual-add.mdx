...
<Info>
  Mem0 automatically uses the latest API version (currently v2), so you can omit the `version` parameter in all requests.
</Info>
...
await client.add(messages1, { user_id: "sarah" });
...
await client.add(messages2, { user_id: "sarah" });
// Mem0 automatically knows Sarah is from New York and can use this context
```
</CodeGroup>
...
await client.add(messages1, { user_id: "user123", run_id: "tokyo-trip-2024" });
...
await client.add(messages2, { user_id: "user123", run_id: "tokyo-trip-2024" });
...
await client.add(workMessages, { user_id: "user123", run_id: "q4-marketing" });
```
</CodeGroup>
