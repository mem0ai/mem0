...
`get` returns category names only. The descriptions you set are used by the classifier but are not read back, so keep your category list in source control if you need to edit it later.
...
```text Output
User's name is Alice ['personal_information']
Alice wants to be more productive at work, maintain a consistent workout routine, and have energy for friends and hobbies ['lifestyle_management_concerns', 'seeking_structure', 'personal_information']
Alice feels overwhelmed trying to balance work, exercise, and social life ['lifestyle_management_concerns', 'personal_information']
```
</CodeGroup>

Alongside `memory` and `categories`, each result carries `id`, `user_id`, `metadata`, `created_at`, `updated_at`, `expiration_date`, and `structured_attributes`.

Categorization runs asynchronously, a moment after the memory itself is written. A memory fetched immediately after `add` can come back with `categories: null` and pick up its tags shortly after.
