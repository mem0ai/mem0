...
Each enhancement can be toggled independently via the `search()` API call. These flags are off by default. These useful when building agents that require fine-grained retrieval control
...
Filtering allows you to narrow down search results by applying specific criterias from the set of retrieved memories.
...
- Combining all modes adds cumulative latency (e.g. ~450ms)
