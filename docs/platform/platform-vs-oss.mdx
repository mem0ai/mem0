...
<AccordionGroup>
  ...
  <Accordion title="Configuration & data operations" icon="sliders">
    | Feature | Platform | Open Source |
    |---------|----------|-------------|
    | **Procedural memory** | Not supported | Python `Memory`/`AsyncMemory` only: pass `memory_type="procedural_memory"` and `agent_id` to `add()` ([details](/core-concepts/memory-types)) |
