...
`GET /configure` redacts saved provider API keys and includes an `api_key_set` boolean alongside each key in `llm.config` and `embedder.config`:
...
The dashboard uses this flag to show that a key is configured. To keep the current key when calling `POST /configure`, omit `api_key`. Empty strings, whitespace-only strings, `null`, and the `[redacted]` placeholder also leave the existing LLM or embedder key unchanged. Send a new key to replace it. `api_key_set` is response metadata and is ignored on save.
