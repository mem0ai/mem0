...
<Info>
  The self-hosted REST server also ships with a web dashboard at `http://localhost:3000`. It covers admin setup, request logs, browsing memories, managing entities (users, agents, runs), API keys, and runtime configuration. See the <Link href="/open-source/features/rest-api">REST API Server</Link> page for setup.
</Info>
