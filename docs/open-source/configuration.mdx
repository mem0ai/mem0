...
<AccordionGroup>
  <Accordion title="Memory management">
    By default, Mem0 uses ADD-only extraction to preserve the full history of stored memories.
    If you want the LLM to resolve conflicting memories during `add()` calls, set
    `enable_memory_management=True` in `MemoryConfig` or include `"enable_memory_management": True`
    in your configuration dictionary.

    When enabled, the extraction step can return `ADD`, `UPDATE`, `DELETE`, or `NONE` actions.
    This lets Mem0 update or remove an existing memory when a new message contradicts, negates,
    or supersedes it. Because this mode allows LLM-driven updates and deletes against the memory
    store, it is opt-in and remains disabled by default.
