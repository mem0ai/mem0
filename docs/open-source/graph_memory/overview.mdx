...
store providers. Currently, we support [Neo4j](#initialize-neo4j), [Memgraph](#initialize-memgraph), [Neptune Analytics](#initialize-neptune-analytics), [Kuzu](#initialize-kuzu), and [Apache AGE](#initialize-apache-age) as graph store providers.
...
[Apache AGE](https://age.apache.org/) is a PostgreSQL extension that provides graph database functionality. It combines the power of PostgreSQL with graph database capabilities, allowing you to use both relational and graph queries on the same data.

#### Prerequisites

Before using Apache AGE with Mem0, you need to:

1. **Install PostgreSQL**: Apache AGE requires PostgreSQL 11 or higher.
2. **Install Apache AGE Extension**: Follow the [Apache AGE installation guide](https://age.apache.org/docs/intro/setup) to install the extension.
3. **Create a Database**: Create a PostgreSQL database where you want to store your graph data.

#### Setup

Once you have PostgreSQL and Apache AGE installed, you can configure Mem0 to use Apache AGE as your graph store:
...
- **graph_name**: Name of the Apache AGE graph (default: "mem0_graph")
- **base_label**: Whether to use base node label `__Entity__` for all entities (optional)

#### Docker Setup

You can also run PostgreSQL with Apache AGE using Docker:

```bash
# Run PostgreSQL with Apache AGE
...
- **SQL + Cypher**: Combine relational SQL queries with graph Cypher queries
- **ACID Compliance**: Full ACID transaction support from PostgreSQL
- **Scalability**: Leverage PostgreSQL's proven scalability and performance
- **Ecosystem**: Use existing PostgreSQL tools and extensions
