...
<Note>
Graph memory is a library feature. The bundled self-hosted FastAPI server under `server/` does not wire up a graph store — configure it in code via `Memory.from_config` as shown below, or run your own REST layer.
</Note>
