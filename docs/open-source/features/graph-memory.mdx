...
Embeddings land in your configured vector database while nodes and edges flow into a graph backend (Neo4j, Memgraph, Neptune, Kuzu, Apache AGE, NebulaGraph).
</Step>
...
Reference: [Apache AGE documentation](https://age.apache.org/age-manual/master/index.html).
  ...
  <Accordion title="NebulaGraph (with external vectors)">
...
m = Memory.from_config(config_dict=config)
```
NebulaGraph v3.8 does not support vector search, you can use Mem0's vector store to store vector data.
Note that graph_store.config.collection_name and vector_store.config.collection_name must be different to avoid interference with each other.

A complete example can be found at [NebulaGraph Example](https://github.com/mem0ai/mem0/blob/main/examples/graph-db-demo/nebulagraph-example.ipynb)

Reference: [NebulaGraph documentation](https://docs.nebula-graph.io/3.8.0/).
  </Accordion>
</AccordionGroup>
