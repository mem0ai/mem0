...
```
  ...
  <Accordion title="Vector-only mode">
    To use only vector memory without graph, simply omit the `graphStore` / `graph_store` section from your config. Graph memory activates automatically when a graph store is configured.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Graph store outage fallback">
    Catch the provider error and create a new `Memory` instance without the `graphStore` / `graph_store` config so vector-only search keeps serving responses while the graph backend recovers.
  </Accordion>
</AccordionGroup>
