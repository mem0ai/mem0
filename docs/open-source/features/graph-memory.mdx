...
<Info icon="lightbulb">
Graph Memory complements—never replaces—your vector store. Keep both healthy to avoid blind spots.
</Info>
...
Reference: [Accessing Data in Neptune DB](https://docs.aws.amazon.com/neptune/latest/userguide/).
  ...
  <Accordion title="Kuzu (embedded)">
    Kuzu runs in-process—just supply a path (or `:memory:`) for the database file:
