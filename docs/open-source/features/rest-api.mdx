...
<Note>
  The container listens on `8000` internally. Docker Compose publishes that as `8888` on the host (so `make bootstrap` → `http://localhost:8888`), while raw `docker run -p 8000:8000` and bare `uvicorn` both stay on `8000`. Use whichever port your own launch command binds.
</Note>
...
| `POST` | `/auth/register` | Register the first admin user; returns 403 once any user exists |
