...
The server supports optional API key authentication. When the `ADMIN_API_KEY` environment variable is set, every endpoint (except the `/` redirect) requires a valid `X-API-Key` header.
