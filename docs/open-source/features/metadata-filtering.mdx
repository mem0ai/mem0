...
Mem0 does not let you choose which metadata fields to index, and an unknown config key is
rejected rather than ignored: `QdrantConfig` runs a `validate_extra_fields` model validator that
raises on any field outside `collection_name`, `embedding_model_dims`, `client`, `host`, `port`,
`url`, `api_key`, `https`, `path` and `on_disk`.
...
What mem0 indexes on its own: `_create_filter_indexes` in `mem0/vector_stores/qdrant.py` calls
`create_payload_index` with a `keyword` schema for `user_id`, `agent_id`, `run_id` and `actor_id`
every time it opens a collection against a Qdrant server. Those four are not configurable, and the
step is skipped when mem0 builds an embedded local client from `path`, which does not support it.

To index your own metadata fields, create the payload indexes on a client and pass that client in:
...
for field in ("category", "priority", "status"):
    client.create_payload_index(collection_name="mem0", field_name=field, field_schema="keyword")
