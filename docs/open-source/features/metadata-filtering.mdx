...
<Info>
This page covers the self-hosted `Memory` / `AsyncMemory` grammar. On self-hosted, sibling top-level keys are implicitly ANDed, so a flat filter like `{"user_id": "alice", "category": "work"}` works without wrapping it in `AND`. The hosted Platform API is stricter: combining more than one condition requires an explicit `AND` / `OR` / `NOT` wrapper, and the `nin` operator documented below is not part of the Platform contract. See [Memory Filters (Platform)](/platform/features/v2-memory-filters) for the hosted grammar, including the exact flat shape the API rejects.
</Info>
...
| --- | --- | --- | --- | --- |
| Qdrant | Full support | Yes | Yes, nested | No-op: matches every record regardless of whether the field is present |
...
| Weaviate | Not implemented: every filter value is matched as an exact equality | Not implemented | Not implemented: only an implicit AND across fields | Not implemented |
...
<CardGroup cols={2}>
  ...
  <Card title="Memory Filters (Platform)" icon="cloud" href="/platform/features/v2-memory-filters">
    Using the hosted API instead? See the stricter AND/OR wrapper grammar, the exact flat shape it rejects, and Platform-only fields like `created_at` and `memory_ids`.
  </Card>
</CardGroup>
