...
<Warning>
  In Python, you must set `enable_vision: True` in your LLM config for image content to be processed. Without it, image turns are silently dropped and no vision memories are created. Example:
  ```python
...
client = Memory.from_config(
    {"llm": {"provider": "openai", "config": {"enable_vision": True, "vision_details": "auto"}}}
)
