...
const Stats = () => {
  ...
  const apps = useSelector((state: RootState) => state.profile.apps).slice(0, 4);
    ...
    <div className="bg-zinc-900 rounded-lg border border-zinc-800">
      ...
      <div className="space-y-3 p-4">
        <div>
          ...
          <h3 className="text-lg font-bold text-white">{totalMemories} Memories</h3>
        </div>
        <div>
          ...
          <div className="flex flex-col items-start gap-1 mt-2">
            <div className="flex -space-x-2">
              {
                apps.map((app) => (
                  <div key={app.id} className={`h-8 w-8 rounded-full bg-primary flex items-center justify-center text-xs`}>
                    <div>
                      <div className="w-7 h-7 rounded-full bg-zinc-700 flex items-center justify-center overflow-hidden">
                        <Image
                          ...
                          alt={constants[app.name as keyof typeof constants]?.name}
                          width={32}
                          height={32}
                        />
                      </div>
