...
const getColor = (label: string): string => {
  ...
  if (l.includes('health') || l.includes('fitness')) return 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20';
  if (l.includes('education') || l.includes('school')) return 'text-indigo-400 bg-indigo-500/10 border-indigo-500/20';
  if (l.includes('business') || l.includes('career') || l.includes('work') || l.includes('finance')) return 'text-amber-400 bg-amber-500/10 border-amber-500/20';
  if (l.includes('design') || l.includes('art') || l.includes('creative')) return 'text-pink-400 bg-pink-500/10 border-pink-500/20';
  if (l.includes('tech') || l.includes('code') || l.includes('programming')) return 'text-purple-400 bg-purple-500/10 border-purple-500/20';
  if (l.includes('interest') || l.includes('preference')) return 'text-rose-400 bg-rose-500/10 border-rose-500/20';
  if (l.includes('travel') || l.includes('trip') || l.includes('location') || l.includes('place')) return 'text-sky-400 bg-sky-500/10 border-sky-500/20';
  if (l.includes('personal') || l.includes('life')) return 'text-yellow-400 bg-yellow-500/10 border-yellow-500/20';
  return 'text-blue-400 bg-blue-500/10 border-blue-500/20';
};

const Categories = ({ categories, isPaused = false, concat = false }: { categories: Category[], isPaused?: boolean, concat?: boolean }) => {
  const [isOpen, setIsOpen] = useState(false);
  
  if (!categories || categories.length === 0) return null;

  const baseBadgeStyle = "backdrop-blur-sm transition-colors hover:bg-opacity-20";
  const pausedStyle = "text-zinc-500 bg-zinc-800/40 border-zinc-700/40 hover:bg-zinc-800/60";
  ...
  if (concat) {
    ...
    return (
      <div className="flex flex-wrap gap-2">
        ...
        <Badge
          variant="outline"
          className={`${isPaused ? pausedStyle : `${getColor(categories[0])} ${baseBadgeStyle}`}`}
        >
          {categories[0]}
        </Badge>
        ...
        {remainingCount > 0 && (
          <Popover open={isOpen} onOpenChange={setIsOpen}>
            ...
            >
              <Badge
                variant="outline"
                className={isPaused ? pausedStyle : "text-zinc-400 bg-zinc-500/10 border-zinc-500/20 hover:bg-zinc-500/20"}
              >
                +{remainingCount}
              </Badge>
            </PopoverTrigger>
              ...
              <div className="flex flex-col gap-2">
                {categories.slice(1).map((cat, i) => (
                  <Badge
                    key={i}
                    variant="outline"
                    className={`${isPaused ? pausedStyle : `${getColor(cat)} ${baseBadgeStyle}`}`}
                  >
                    {cat}
                  </Badge>
                ))}
