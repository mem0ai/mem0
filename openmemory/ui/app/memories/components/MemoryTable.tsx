...
import { selectMemory, deselectMemory, selectAllMemories, clearSelection } from "@/store/memoriesSlice"
  ...
  return (
    <div className="rounded-md border">
      <Table className="">
        ...
        <TableBody>
          {memories.map((memory) => (
            <TableRow key={memory.id} className={`hover:bg-zinc-900/50 ${memory.state === "paused" || memory.state === "archived" ? "text-zinc-400" : ""} ${isLoading ? "animate-pulse opacity-50" : ""}`}>
                  ...
                  <Tooltip delayDuration={0}>
                    <TooltipTrigger asChild>
                      <div
                        ...
                        className={`font-medium ${memory.state === "paused" || memory.state === "archived" ? "text-zinc-400" : "text-white"} cursor-pointer`}
                      >
                        {memory.memory}
                      </div>
                    </TooltipTrigger>
                    <TooltipContent>
                      <p>This memory is <span className="font-bold">{memory.state === "paused" ? "paused" : "archived"}</span> and <span className="font-bold">disabled</span>.</p>
                    </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                ) : (
              ...
              <TableCell className="w-[140px] text-center">{formatDate(memory.created_at)}</TableCell>
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="bg-zinc-900 border-zinc-800">
                    <DropdownMenuItem
                      ...
                      onClick={() => {
                        const newState = memory.state === "active" ? "paused" : "active";
                    ...
                    <DropdownMenuItem className="cursor-pointer" onClick={() => handleEditMemory(memory.id, memory.memory)}>
