...
import { setSearchQuery, setActiveFilter, setSortBy, setSortDirection } from "@/store/appsSlice"
...
export function AppFilters() {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <Select value={String(filters.isActive)} onValueChange={handleActiveFilterChange}>
        ...
        <DropdownMenuContent className="w-56 bg-zinc-900 border-zinc-800 text-zinc-100">
          ...
          <DropdownMenuGroup>
            {sortOptions.map((option) => (
              <DropdownMenuItem
                key={option.value}
                onClick={() => setSorting(option.value as 'name' | 'memories' | 'memories_accessed')}
