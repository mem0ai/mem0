...
class QualityEvalTests(unittest.TestCase):
    ...
    def test_run_quality_eval_passes_golden_recall_scenarios(self) -> None:
        ...
        self.assertEqual(report["metrics"]["required_hit_rate"], 1.0)
        self.assertEqual(report["metrics"]["forbidden_leak_rate"], 0.0)
        self.assertGreater(report["metrics"]["avg_selected_count"], 0.0)
        self.assertEqual(report["metrics"]["mean_scenario_score"], 1.0)
        self.assertTrue(all(result["passed"] for result in report["results"]))
        self.assertTrue(all(result["required_hit_rate"] == 1.0 for result in report["results"]))
        self.assertTrue(all(result["forbidden_leak_rate"] == 0.0 for result in report["results"]))
