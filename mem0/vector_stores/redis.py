...
class RedisDB(VectorStoreBase):
    ...
    def search(self, query: list, limit: int = 5, filters: dict = None):
        ...
        return [MemoryResult(id=result["memory_id"],
                             score=result["vector_distance"],
                            payload={
                                "hash": result["hash"],
                                "data": result["memory"],
                                "created_at": datetime.fromtimestamp(int(result["created_at"]), tz=pytz.timezone('US/Pacific')).isoformat(timespec='microseconds'),
                                **({"updated_at": datetime.fromtimestamp(int(result["updated_at"]), tz=pytz.timezone('US/Pacific')).isoformat(timespec='microseconds')} if "updated_at" in result else {}),
                                **{field: result[field] 
                                   for field in ["agent_id", "run_id", "user_id"] 
                                   if field in result},
    ...
    def list(self, filters: dict = None, limit: int = None) -> list:
        ...
        return [[MemoryResult(id=result["memory_id"],
                            payload={
                                ...
                                "data": result["memory"],
                                "created_at": datetime.fromtimestamp(int(result["created_at"]), tz=pytz.timezone('US/Pacific')).isoformat(timespec='microseconds'),
                                **({"updated_at": datetime.fromtimestamp(int(result["updated_at"]), tz=pytz.timezone('US/Pacific')).isoformat(timespec='microseconds')} if result.__dict__.get("updated_at") else {}),
                                **{field: result[field] 
                                   for field in ["agent_id", "run_id", "user_id"] 
