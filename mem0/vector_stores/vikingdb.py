...
try:
    ...
    raise ImportError(
        "The 'volcengine' library is required. Please install it using 'pip install volcengine'."
    )
...
from volcengine.viking_db import (
    ...
    ScalarOrder,
    Order,
    Field as VikingDBField,
    FieldType as VikingDBFieldType,
)
...
DEFAULT_ATTR_FIELDS = [
    VikingDBField(
        field_name="hash", field_type=VikingDBFieldType.String, default_val=""
    ),
    VikingDBField(
        field_name="data", field_type=VikingDBFieldType.String, default_val=""
    ),
    VikingDBField(
        field_name="metadata", field_type=VikingDBFieldType.String, default_val=""
    ),
]

DEFAULT_FILTER_FIELDS = [
    VikingDBField(
        field_name="agent_id", field_type=VikingDBFieldType.String, default_val=""
    ),
    VikingDBField(
        field_name="run_id", field_type=VikingDBFieldType.String, default_val=""
    ),
    VikingDBField(
        field_name="user_id", field_type=VikingDBFieldType.String, default_val=""
    ),
    VikingDBField(
        field_name="created_at", field_type=VikingDBFieldType.String, default_val=""
    ),
    VikingDBField(
        field_name="updated_at", field_type=VikingDBFieldType.String, default_val=""
    ),
]
...
class VikingDB(VectorStoreBase):
    ...
    def create_col(
        ...
        if self._has_collection():
            ...
            fields = [
                VikingDBField(
                    field_name="id",
                    field_type=VikingDBFieldType.String,
                    is_primary_key=True,
                ),
                VikingDBField(
                    field_name="vectors",
                    field_type=VikingDBFieldType.Vector,
                    dim=vector_size,
                ),
            ]
            ...
            self.index = self.client.create_index(
                ...
                vector_index=VectorIndexParams(
                    distance=METRIC_TO_DISTANCE[metric_type]
                ),
    ...
    def _payload_to_fields(self, payload) -> Dict:
        ...
        excluded_fields["metadata"] = json.dumps(
    ...
    def insert(self, ids, vectors, payloads):
        ...
        for idx, vector, payload in zip(ids, vectors, payloads):
            ...
            fields.update(self._payload_to_fields(payload))
    ...
    def list(self, filters: dict = None, limit: int = 100) -> list:
        ...
        """
        query_filter = self._create_filter(filters) if filters else None
        data = self.index.search(filter=query_filter, limit=limit)
        result = self._parse_output(data=data)
