...
class MemoryGraph:
    ...
    def add(self, data, filters):
        ...
        added_entities = self._add_entities(to_be_added, filters.get("user_id"), entity_type_map, filters.get("agent_id"))
    ...
    def _establish_nodes_relations_from_data(self, data, filters, entity_type_map):
        ...
        if self.config.graph_store.custom_prompt:
            messages = [
                {
                    "role": "system",
                    "content": EXTRACT_RELATIONS_PROMPT.replace("USER_ID", user_id).replace(
            ...
            messages = [
                {
                    "role": "system",
                    "content": EXTRACT_RELATIONS_PROMPT.replace("USER_ID", user_id),
                },
    ...
    def _search_graph_db(self, node_list, filters, limit=100):
        ...
        for node in node_list:
            ...
            if filters.get("user_id"):
                cypher_query += "AND n.user_id = $user_id "
                params["user_id"] = filters["user_id"]
            
            if filters.get("agent_id"):
                cypher_query += "AND n.agent_id = $agent_id "
                params["agent_id"] = filters["agent_id"]
            ...
            if filters.get("user_id"):
                destination_conditions.append("m.user_id = $user_id")
            
            if filters.get("agent_id"):
                destination_conditions.append("m.agent_id = $agent_id")
            
            if destination_conditions:
                cypher_query += "WHERE " + " AND ".join(destination_conditions) + " "
            ...
            source_conditions = []
            if filters.get("user_id"):
                source_conditions.append("m.user_id = $user_id")
            
            if filters.get("agent_id"):
                source_conditions.append("m.agent_id = $agent_id")
            
            if source_conditions:
                cypher_query += "WHERE " + " AND ".join(source_conditions) + " "
    ...
    def _delete_entities(self, to_be_deleted, user_id=None, agent_id=None):
        ...
        for item in to_be_deleted:
            ...
            cypher = "MATCH (n {name: $source_name})-[r:" + relationship + "]->(m {name: $dest_name}) WHERE 1=1 "
            params = {
                "source_name": source,
    ...
    def _add_entities(self, to_be_added, user_id=None, entity_type_map=None, agent_id=None):
        ...
        for item in to_be_added:
            ...
            source_node_search_result = self._search_source_node(source_embedding, user_id, agent_id, threshold=0.9)
            destination_node_search_result = self._search_destination_node(dest_embedding, user_id, agent_id, threshold=0.9)
            ...
            if not destination_node_search_result and source_node_search_result:
                cypher = f"""
                    ...
                    WITH source
                    MERGE (destination:{destination_type} {dest_props})
                    ON CREATE SET
                ...
                cypher = f"""
                    ...
                    WITH destination
                    MERGE (source:{source_type} {source_props})
                    ON CREATE SET
                ...
                cypher = f"""
                    MERGE (n:{source_type} {source_props})
                    ...
                    WITH n
                    MERGE (m:{destination_type} {dest_props})
    ...
    def _search_source_node(self, source_embedding, user_id=None, agent_id=None, threshold=0.9):
        ...
        if user_id:
            cypher += "AND source_candidate.user_id = $user_id "
            params["user_id"] = user_id
        
        if agent_id:
            cypher += "AND source_candidate.agent_id = $agent_id "
            params["agent_id"] = agent_id
    ...
    def _search_destination_node(self, destination_embedding, user_id=None, agent_id=None, threshold=0.9):
        ...
        if user_id:
            cypher += "AND destination_candidate.user_id = $user_id "
            params["user_id"] = user_id
        
        if agent_id:
            cypher += "AND destination_candidate.agent_id = $agent_id "
            params["agent_id"] = agent_id
