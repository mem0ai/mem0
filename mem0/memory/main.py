...
class Memory(MemoryBase):
    ...
    def _bulk_clear_entity_store(self, filters):
        ...
        sequential _delete_memory calls each rewrite the same entity rows'
        linked_memory_ids lists. Mirrors the async variant.
        """
        if self._entity_store is None:
            return
        search_filters = {k: v for k, v in filters.items() if k in ("user_id", "agent_id", "run_id") and v}
    ...
    def delete_all(self, user_id: Optional[str] = None, agent_id: Optional[str] = None, run_id: Optional[str] = None):
        ...
        for memory in memories:
            try:
                # Skip per-memory entity cleanup; the entity store is cleared in
                # one bulk pass below to avoid a read-modify-write race across
                # many sequential deletes.
        ...
        logger.info(f"Deleted {len(memories) - len(errors)} memories")
    ...
    def _delete_memory(self, memory_id, existing_memory=None, skip_entity_cleanup=False):
        ...
        # delete_all, which clears the entity store in one pass to avoid a
        # read-modify-write race across many sequential deletes.
