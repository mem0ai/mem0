...
class Memory(MemoryBase):
    ...
    def delete_all(self, user_id: Optional[str] = None, agent_id: Optional[str] = None, run_id: Optional[str] = None):
        ...
        # Fetch all matching memories in batches to avoid the default page-size cap
        all_memories = []
        batch_size = 500
        while True:
        ...
        logger.info(f"Deleted {len(all_memories)} memories")
...
class AsyncMemory(MemoryBase):
    ...
    async def delete_all(self, user_id=None, agent_id=None, run_id=None):
        ...
        batch_size = 500
        while True:
            batch = (await asyncio.to_thread(self.vector_store.list, filters=filters, top_k=batch_size))[0]
            all_memories.extend(batch)
            if len(batch) < batch_size:
                break

        delete_tasks = [self._delete_memory(memory.id) for memory in all_memories]
