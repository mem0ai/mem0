...
class MoonshotLLM(LLMBase):
    def __init__(self, config: Optional[Union[BaseLlmConfig, MoonshotConfig, Dict]] = None):
        if config is None:
            config = MoonshotConfig()
        elif isinstance(config, dict):
            config = MoonshotConfig(**config)
        elif isinstance(config, BaseLlmConfig) and not isinstance(config, MoonshotConfig):
            config = MoonshotConfig(
                model=config.model,
