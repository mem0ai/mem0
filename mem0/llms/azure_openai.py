...
class AzureOpenAILLM(LLMBase):
    ...
    def _rewrite_assistant_keyword(messages):
        ...
        The replacement is anchored on word boundaries so that unrelated words
        that merely contain the substring "assistant" (e.g. "assistance") are
        left intact (see issue #6036).
        """
        if not messages:
            return messages
