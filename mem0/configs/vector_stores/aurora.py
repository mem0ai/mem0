...
class AuroraConfig(BaseModel):
    ...
    index_type: Optional[str] = Field("hnsw", description="Use hnsw for faster search")
    distance: Optional[str] = Field("cosine", description="Distance metric to use. Defaults to None, which uses 'cosine'")
    ...
    hnsw_m: Optional[int] = Field(16, description="HNSW M parameter (connections/edges per node). Defaults to 16.")
    hnsw_ef_construction: Optional[int] = Field(200, description="HNSW ef_construction parameter. Defaults to 200.")
    hnsw_iterative_scan: Optional[str] = Field("relaxed_order", description="automatically scan more of the index when needed for hnsw. Defaults to enabled.")
    ivfflat_iterative_scan: Optional[str] = Field("relaxed_order", description="automatically scan more of the index when needed for ivfflat. Defaults to enabled.")
    ivfflat_list: Optional[int] = Field(100, description="An IVFFlat index divides vectors into lists and then searches a subset of those lists that are closest to the query vector.Default to be 100.")

    max_parallel_maintenance_workers: Optional[int] = Field(2, description="Sets the maximum number of parallel workers that can be started by a single utility command. Defaults to 2.")
