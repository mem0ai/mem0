...
Given a memory $m_i$ with base strength $S_{base}$ and emotional valence $E_i \in [0, 1]$ (extracted via LLM prompt; see Appendix~\ref{app:prompts}), the effective retention strength is:
\begin{equation}
    S_{effective}(m_i) = S_{base} \cdot (1 + \lambda \cdot E_i)
\end{equation}
where $\lambda$ controls the \textit{emotional inertia}. The retention probability at time $\Delta t$ after encoding is then:
\begin{equation}
    R(m_i, \Delta t) = \exp\left(-\frac{\Delta t}{S_{base} \cdot (1 + \lambda \cdot E_i)}\right)
\end{equation}
...
\begin{equation}
    \text{Score}(m_i) = \alpha \cdot R(m_i, \Delta t) + \beta \cdot \text{norm}(f_i) + \gamma \cdot w_{recency}
\end{equation}
where $\alpha, \beta, \gamma$ are hyperparameters balancing the contributions.
