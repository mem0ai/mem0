...
try:
    ...
    raise ImportError(
        "Please install the langchain_nebius package by running `pip install langchain_nebius==0.1.3`."
    )
...
class NebiusLlm(BaseLlm):
    def __init__(self, config: Optional[BaseLlmConfig] = None):
        try:
            importlib.import_module("nebius")
        except ModuleNotFoundError:
            raise ModuleNotFoundError(
                "The required dependencies for Nebius are not installed."
                'Please install with `pip install --upgrade "embedchain[nebius]"`'
            ) from None

        super().__init__(config=config)
        if not self.config.api_key and "NEBIUS_API_KEY" not in os.environ:
            raise ValueError("Please set the NEBIUS_API_KEY environment variable or pass it in the config.")

    def get_llm_model_answer(self, prompt) -> tuple[str, Optional[dict[str, Any]]]:
        if self.config.system_prompt:
            raise ValueError("NebiusLlm does not support `system_prompt`")

        if self.config.token_usage:
            response, token_info = self._get_answer(prompt, self.config)
            model_name = "nebius/" + self.config.model
