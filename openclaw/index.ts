...
const memoryPlugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    // Track the latest session ID for CLI/session-scoped fallbacks.
    // Tools receive ctx directly and should prefer ctx.sessionKey/ctx.agentId.
    ...
    const _resolveUserId = (
      ...
      if (opts.agentId) return _agentUserId(opts.agentId);
      if (opts.userId) return opts.userId;
      if (ctx?.agentId) return _agentUserId(ctx.agentId);
      return resolveUserId(cfg.userId, {}, ctx?.sessionKey ?? currentSessionId);
    };
    ...
    registerTools(api, provider, cfg, _resolveUserId, buildAddOptions, buildSearchOptions);
      ...
      async execute(_toolCallId, params) {
        ...
        try {
          ...
          const uid = _resolveUserId(
            { agentId, userId },
            { agentId: ctx.agentId, sessionKey },
          );
  ...
  api.registerCli(
    ({ program }) => {
      ...
      mem0
        ...
        .option("--session <sessionKey>", "Session key for session-scoped search")
        .action(async (query: string, opts: { limit: string; scope: string; agent?: string; session?: string }) => {
          try {
            ...
            const currentSessionId = opts.session ?? getCurrentSessionId();
            ...
            if (scope === "session" || scope === "all") {
              if (currentSessionId) {
                ...
                console.log("No active session ID available for session-scoped search. Pass --session <sessionKey>.");
                return;
              }
            }
