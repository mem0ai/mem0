...
@patch('mem0.utils.factory.LlmFactory.create')
@patch('mem0.memory.storage.SQLiteManager')
def test_search_returns_empty_for_empty_query(mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory):
    """search() should return empty results for an empty query string."""
    mock_embedder_factory.return_value = MagicMock()
    mock_vector_factory.return_value = MagicMock()
    mock_llm_factory.return_value = MagicMock()
    mock_sqlite.return_value = MagicMock()
    ...
    result = memory.search("", filters={"user_id": "u1"})
    assert result == {"results": []}
    # Ensure the vector store was never called
    mock_embedder_factory.return_value.embed.assert_not_called()
...
@patch('mem0.utils.factory.LlmFactory.create')
@patch('mem0.memory.storage.SQLiteManager')
def test_search_returns_empty_for_whitespace_query(mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory):
    """search() should return empty results for a whitespace-only query."""
    mock_embedder_factory.return_value = MagicMock()
    mock_vector_factory.return_value = MagicMock()
    mock_llm_factory.return_value = MagicMock()
    mock_sqlite.return_value = MagicMock()
...
@patch('mem0.utils.factory.LlmFactory.create')
@patch('mem0.memory.storage.SQLiteManager')
def test_search_returns_empty_for_none_query(mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory):
    """search() should return empty results for a None query."""
    mock_embedder_factory.return_value = MagicMock()
    mock_vector_factory.return_value = MagicMock()
    mock_llm_factory.return_value = MagicMock()
    mock_sqlite.return_value = MagicMock()
    ...
    result = memory.search(None, filters={"user_id": "u1"})
    assert result == {"results": []}
    mock_embedder_factory.return_value.embed.assert_not_called()
