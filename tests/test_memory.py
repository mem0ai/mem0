...
@patch('mem0.utils.factory.GraphStoreFactory.create')
@patch('mem0.utils.factory.EmbedderFactory.create')
@patch('mem0.utils.factory.VectorStoreFactory.create')
@patch('mem0.utils.factory.LlmFactory.create')
@patch('mem0.memory.storage.SQLiteManager')
def test_delete_memory_calls_graph_cleanup(mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory, mock_graph_factory):
    """Test that deleting a memory triggers graph cleanup when graph is enabled"""
    # Setup mocks
    mock_embedder_factory.return_value = MagicMock()
    mock_vector_store = MagicMock()
    mock_vector_factory.return_value = mock_vector_store
    ...
    # Configure vector store to return memory with payload
    mock_memory = MagicMock()
    mock_memory.payload = {
        "data": "John loves Python",
        "user_id": "test_user",
        "agent_id": "test_agent"
    }
    mock_vector_store.get.return_value = mock_memory
    
    # Create memory instance with graph enabled
    config = MemoryConfig()
    config.graph_store.config = MagicMock()
    ...
    # Verify graph.delete was called with correct parameters
    mock_graph.delete.assert_called_once()
    call_args = mock_graph.delete.call_args
    assert call_args[0][0] == "John loves Python"  # data argument
    assert call_args[0][1]["user_id"] == "test_user"  # filters argument
    assert call_args[0][1]["agent_id"] == "test_agent"
    
    # Verify vector store delete was still called
    mock_vector_store.delete.assert_called_once_with(vector_id="test_memory_id")
...
@patch('mem0.utils.factory.LlmFactory.create')
@patch('mem0.memory.storage.SQLiteManager')
def test_delete_memory_continues_without_graph(mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory):
    """Test that memory deletion continues even when graph is not enabled"""
    # Setup mocks
    mock_embedder_factory.return_value = MagicMock()
    mock_vector_store = MagicMock()
    mock_vector_factory.return_value = mock_vector_store
    ...
    # Create memory without graph
    config = MemoryConfig()
    memory = Memory(config)
    
    # Delete should work without graph
    result = memory.delete("test_memory_id")
