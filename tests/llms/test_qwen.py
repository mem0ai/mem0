...
def test_qwen_llm_base_url():
    # case1: default config with Qwen official base url
    config = BaseLlmConfig(model="qwen-plus", temperature=0.7, max_tokens=100, top_p=1.0, api_key="api_key")
    llm = QwenLLM(config)
    assert str(llm.client.base_url) == "https://dashscope.aliyuncs.com/compatible-mode/v1/"

    # case2: with env variable QWEN_API_BASE
    provider_base_url = "https://api.provider.com/v1/"
    os.environ["QWEN_API_BASE"] = provider_base_url
    config = QwenConfig(model="qwen-plus", temperature=0.7, max_tokens=100, top_p=1.0, api_key="api_key")
    llm = QwenLLM(config)
    ...
    config = QwenConfig(
        ...
        top_p=1.0,
        api_key="api_key",
        qwen_base_url=config_base_url,
    )
    llm = QwenLLM(config)
    assert str(llm.client.base_url) == config_base_url


def test_generate_response_without_tools(mock_qwen_client):
    config = BaseLlmConfig(model="qwen-plus", temperature=0.7, max_tokens=100, top_p=1.0)
    llm = QwenLLM(config)
    messages = [
    ...
    mock_qwen_client.chat.completions.create.assert_called_once_with(
        model="qwen-plus", messages=messages, temperature=0.7, max_tokens=100, top_p=1.0
    )
    assert response == "I'm doing well, thank you for asking!"


def test_generate_response_with_tools(mock_qwen_client):
    config = BaseLlmConfig(model="qwen-plus", temperature=0.7, max_tokens=100, top_p=1.0)
    llm = QwenLLM(config)
    messages = [
        {"role": "system", "content": "You are a helpful assistant."},
...
def test_generate_response_with_response_format(mock_qwen_client):
    config = BaseLlmConfig(model="qwen-plus", temperature=0.7, max_tokens=100, top_p=1.0)
    llm = QwenLLM(config)
    messages = [
...
def test_generate_response_without_response_format(mock_qwen_client):
    config = BaseLlmConfig(model="qwen-plus", temperature=0.7, max_tokens=100, top_p=1.0)
    llm = QwenLLM(config)
    messages = [
        {"role": "system", "content": "You are a helpful assistant."},
