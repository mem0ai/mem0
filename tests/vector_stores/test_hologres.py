...
from mem0.vector_stores.hologres import Hologres


class TestHologres(unittest.TestCase):
    def setUp(self):
        ...
        self.connection_string = "postgresql://user:pass@host:80/db"
    ...
    @patch('mem0.vector_stores.hologres.PSYCOPG_VERSION', 3)
    @patch('mem0.vector_stores.hologres.ConnectionPool')
    def test_init_with_individual_params_psycopg3(self, mock_psycopg_pool):
        ...
        mock_psycopg_pool.assert_called_once_with(
            conninfo="postgresql://test_user:test_pass@localhost:80/test_db?application_name=mem0",
            min_size=1,
            max_size=4,
            open=True,
        )
        self.assertEqual(hologres.collection_name, "test_collection")
        self.assertEqual(hologres.embedding_model_dims, 3)

    @patch('mem0.vector_stores.hologres.PSYCOPG_VERSION', 2)
    @patch('mem0.vector_stores.hologres.ConnectionPool')
    def test_init_with_individual_params_psycopg2(self, mock_pcycopg2_pool):
        ...
        mock_pcycopg2_pool.assert_called_once_with(
            minconn=1,
            maxconn=4,
            dsn="postgresql://test_user:test_pass@localhost:80/test_db?application_name=mem0",
        )
    ...
    @patch('mem0.vector_stores.hologres.PSYCOPG_VERSION', 3)
    @patch('mem0.vector_stores.hologres.ConnectionPool')
    @patch.object(Hologres, '_get_cursor')
    def test_create_col_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        # Verify vector index and table creation
        ...
        index_creation_calls = [call for call in self.mock_cursor.execute.call_args_list 
                              if "vectors = \\\'{" in str(call)]
        self.assertTrue(len(table_creation_calls) > 0)
        self.assertTrue(len(index_creation_calls) > 0)
        
        # Verify hologres instance properties
        self.assertEqual(hologres.collection_name, "test_collection")
        self.assertEqual(hologres.embedding_model_dims, 3)
    ...
    def test_create_col_psycopg3_with_explicit_pool(self, mock_get_cursor, mock_connection_pool):
        ...
        This ensures that Hologres uses the provided pool and still performs collection creation logic.
        """
        ...
        # Pass the explicit pool to Hologres
        hologres = Hologres(
            dbname="test_db",
            collection_name="test_collection",
            embedding_model_dims=3,
        ...
        # Verify vector index and table creation
        all_calls = [str(call) for call in self.mock_cursor.execute.call_args_list]
        table_creation_calls = [call for call in self.mock_cursor.execute.call_args_list 
                              if "CREATE TABLE IF NOT EXISTS test_collection" in str(call)]
        index_creation_calls = [call for call in self.mock_cursor.execute.call_args_list 
                              if "vectors = \\\'{" in str(call)]
        ...
        self.assertIs(hologres.connection_pool, explicit_pool)
    ...
    def test_insert_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        hologres.insert(self.test_vectors, self.test_payloads, self.test_ids)
    ...
    def test_insert_psycopg2(self, mock_get_cursor, mock_connection_pool):
        ...
        This test ensures that Hologres.insert uses psycopg2.extras.execute_values for batch inserts
        ...
        # Patch sys.modules so that imports in Hologres use our mocks
        with patch.dict('sys.modules', {
            ...
            # Force reload of Hologres to pick up the mocked modules
            if 'mem0.vector_stores.hologres' in sys.modules:
                importlib.reload(sys.modules['mem0.vector_stores.hologres'])
    ...
    def test_search_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        results = hologres.search("test query", [0.1, 0.2, 0.3], limit=2)
        ...
        search_calls = [call for call in self.mock_cursor.execute.call_args_list 
                       if "SELECT id, approx_cosine_distance(vector," in str(call)]
    ...
    def test_delete_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        hologres.delete(self.test_ids[0])
    ...
    def test_update_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        hologres.update(self.test_ids[0], vector=updated_vector, payload=updated_payload)
    ...
    def test_get_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        result = hologres.get(self.test_ids[0])
    ...
    def test_list_cols_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        collections = hologres.list_cols()
    ...
    def test_col_info_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        self.mock_cursor.fetchall.return_value = []  # No existing collections
        self.mock_cursor.fetchone.side_effect = [[100], ["1 MB"]]
        ...
        info = hologres.col_info()
        
        # Verify the _get_cursor context manager was called
        mock_get_cursor.assert_called()
        
        # Verify col_info query was executed
        count_calls = [call for call in self.mock_cursor.execute.call_args_list 
                     if "SELECT COUNT(*) FROM" in str(call)]
        size_calls = [call for call in self.mock_cursor.execute.call_args_list 
                     if "SELECT pg_size_pretty(pg_total_relation_size(" in str(call)]
        self.assertTrue(len(count_calls) > 0)
        self.assertTrue(len(size_calls) > 0)
    ...
    def test_search_with_filters_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        results = hologres.search("test query", [0.1, 0.2, 0.3], limit=2, filters=filters)
        ...
        search_calls = [call for call in self.mock_cursor.execute.call_args_list 
                       if "SELECT id, approx_cosine_distance(vector," in str(call) and "WHERE" in str(call)]
        self.assertTrue(len(search_calls) > 0)
    ...
    def test_search_with_no_filters_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        results = hologres.search("test query", [0.1, 0.2, 0.3], limit=2, filters=None)
        ...
        search_calls = [call for call in self.mock_cursor.execute.call_args_list 
                       if "SELECT id, approx_cosine_distance(vector," in str(call) and "WHERE" not in str(call)]
        self.assertTrue(len(search_calls) > 0)
    ...
    def test_list_with_filters_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        results = hologres.list(filters=filters, limit=2)
    ...
    def test_list_with_no_filters_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        results = hologres.list(filters=None, limit=2)
    ...
    @patch('mem0.vector_stores.hologres.ConnectionPool')
    @patch.object(Hologres, '_get_cursor')
    @patch('mem0.vector_stores.hologres.Json')
    def test_update_payload_psycopg3_json_handling(self, mock_json, mock_get_cursor, mock_connection_pool):
        ...
        hologres.update("test-id-123", payload=test_payload)
    ...
    def test_pool_connection_error_handling(self, mock_get_cursor, mock_connection_pool):
        ...
        # Use a flag to only raise the exception after Hologres is initialized
        ...
        with self.assertRaises(Exception) as context:
            hologres.search("test query", [0.1, 0.2, 0.3])
    ...
    def test_update_vector_only_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        hologres.update("test-id", vector=test_vector)
    ...
    def test_update_both_vector_and_payload_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        hologres.update("test-id", vector=test_vector, payload=test_payload)
    ...
    def test_connection_string_with_sslmode_psycopg3(self, mock_connection_pool):
        ...
        connection_string = "postgresql://user:pass@localhost:80/db"
        ...
        expected_conn_string = f"{connection_string}?sslmode=require&application_name=mem0"
    ...
    def test_pool_cleanup_psycopg3(self):
        ...
        with patch('mem0.vector_stores.hologres.PSYCOPG_VERSION', 3), \
             patch('mem0.vector_stores.hologres.ConnectionPool') as mock_connection_pool:
    ...
    def test_pool_cleanup_psycopg2(self):
        ...
        with patch('mem0.vector_stores.hologres.PSYCOPG_VERSION', 2), \
             patch('mem0.vector_stores.hologres.ConnectionPool') as mock_connection_pool:
