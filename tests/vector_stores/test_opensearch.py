...
class TestOpenSearchDB(unittest.TestCase):
    ...
    def test_create_index(self):
        ...
        self.assertEqual(
            mappings["vector_field"]["method"],
            {"engine": "lucene", "name": "hnsw", "space_type": "cosinesimil"},
        )
...
def test_opensearch_config_accepts_knn_engine_and_space_type():
    config = OpenSearchConfig(host="localhost", knn_engine="faiss", knn_space_type="l2")

    assert config.knn_engine == "faiss"
    assert config.knn_space_type == "l2"
