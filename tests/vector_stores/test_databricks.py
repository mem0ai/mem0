...
# Ensure repository root (containing local mem0 package) is at front of sys.path
...
def mock_workspace_client():
	...
	with patch("mem0.vector_stores.databricks.WorkspaceClient") as mock_wc_cls:
		...
		mock_wc.vector_search_indexes.query_index.return_value = SimpleNamespace(
...
def test_initialization_delta_sync(db_instance_delta, mock_workspace_client):
	...
	assert mock_workspace_client.vector_search_indexes.create_index.call_args.kwargs["index_type"] == VectorIndexType.DELTA_SYNC
...
def test_initialization_direct_access(db_instance_direct, mock_workspace_client):
	...
	assert mock_workspace_client.vector_search_indexes.create_index.call_args.kwargs["index_type"] == VectorIndexType.DIRECT_ACCESS
