...
from mem0.vector_stores.aurora import Aurora
...
class TestAurora(unittest.TestCase):
    ...
    @patch('mem0.vector_stores.aurora.PSYCOPG_VERSION', 3)
    @patch('mem0.vector_stores.aurora.ConnectionPool')
    def test_init_with_individual_params_psycopg3(self, mock_psycopg_pool):
        ...
        self.assertEqual(aurora.collection_name, "test_collection")
        self.assertEqual(aurora.embedding_model_dims, 3)

    @patch('mem0.vector_stores.aurora.PSYCOPG_VERSION', 2)
    @patch('mem0.vector_stores.aurora.ConnectionPool')
    ...
    @patch('mem0.vector_stores.aurora.PSYCOPG_VERSION', 3)
    @patch('mem0.vector_stores.aurora.ConnectionPool')
    @patch.object(Aurora, '_get_cursor')
    def test_create_col_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        # Verify aurora instance properties
        self.assertEqual(aurora.collection_name, "test_collection")
        self.assertEqual(aurora.embedding_model_dims, 3)
    ...
    def test_create_col_psycopg3_with_explicit_pool(self, mock_get_cursor, mock_connection_pool):
        ...
        This ensures that Aurora uses the provided pool and still performs collection creation logic.
        """
        ...
        # Pass the explicit pool to Aurora
        aurora = Aurora(
            dbname="test_db",
            collection_name="test_collection",
            embedding_model_dims=3,
        ...
        self.assertIs(aurora.connection_pool, explicit_pool)
    ...
    def test_insert_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        aurora.insert(self.test_vectors, self.test_payloads, self.test_ids)
    ...
    def test_insert_psycopg2(self, mock_get_cursor, mock_connection_pool):
        ...
        This test ensures that Aurora.insert uses psycopg2.extras.execute_values for batch inserts
        ...
        # Patch sys.modules so that imports in Aurora use our mocks
        with patch.dict('sys.modules', {
            ...
            # Force reload of Aurora to pick up the mocked modules
            if 'mem0.vector_stores.aurora' in sys.modules:
                importlib.reload(sys.modules['mem0.vector_stores.aurora'])
    ...
    @patch('mem0.vector_stores.aurora.ConnectionPool')
    @patch.object(Aurora, '_get_cursor')
    def test_search_psycopg3_all_distances(self, mock_get_cursor, mock_connection_pool):
        """Test search operator across distances with psycopg3 using subTest."""
        # Set up mock pool and cursor
        mock_pool = MagicMock()
        mock_connection_pool.return_value = mock_pool
        ...
        distances = ["cosine", "L1", "L2", "IP", "hamming", "jaccard"]
        for dist in distances:
            with self.subTest(distance=dist):
                ...
                results = aurora.search("test query", [0.1, 0.2, 0.3], limit=2)
                # Verify the _get_cursor context manager was called
                mock_get_cursor.assert_called()
                # Verify search query was executed
                op = aurora.vector_ops[aurora.distance] 
                search_calls = [call for call in self.mock_cursor.execute.call_args_list 
                       if f"SELECT id, vector {op}" in str(call)]
                print(search_calls)
    ...
    @patch('mem0.vector_stores.aurora.ConnectionPool')
    @patch.object(Aurora, '_get_cursor')
    def test_search_psycopg2_all_distances(self, mock_get_cursor, mock_connection_pool):
    ...
    def test_update_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        aurora.update(self.test_ids[0], vector=updated_vector, payload=updated_payload)
    ...
    def test_get_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        result = aurora.get(self.test_ids[0])
    ...
    def test_search_with_filters_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        results = aurora.search("test query", [0.1, 0.2, 0.3], limit=2, filters=filters)
    ...
    def test_search_with_no_filters_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        results = aurora.search("test query", [0.1, 0.2, 0.3], limit=2, filters=None)
    ...
    def test_list_with_filters_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        results = aurora.list(filters=filters, limit=2)
    ...
    def test_list_with_no_filters_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        results = aurora.list(filters=None, limit=2)
    ...
    @patch('mem0.vector_stores.aurora.ConnectionPool')
    @patch.object(Aurora, '_get_cursor')
    @patch('mem0.vector_stores.aurora.Json')
    def test_update_payload_psycopg3_json_handling(self, mock_json, mock_get_cursor, mock_connection_pool):
        ...
        aurora.update("test-id-123", payload=test_payload)
    ...
    def test_pool_connection_error_handling(self, mock_get_cursor, mock_connection_pool):
        ...
        # Use a flag to only raise the exception after Aurora is initialized
        ...
        with self.assertRaises(Exception) as context:
            aurora.search("test query", [0.1, 0.2, 0.3])
    ...
    def test_update_vector_only_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        aurora.update("test-id", vector=test_vector)
    ...
    def test_update_both_vector_and_payload_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        aurora.update("test-id", vector=test_vector, payload=test_payload)
    ...
    def test_create_col_with_hnsw_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        self.assertEqual(aurora.collection_name, "test_collection")
        self.assertEqual(aurora.embedding_model_dims, 3)
        self.assertEqual(aurora.hnsw_m, 16)
        self.assertEqual(aurora.hnsw_ef_construction, 200)

        calls_str = [str(c) for c in self.mock_cursor.execute.call_args_list]
        self.assertTrue(any("SET hnsw.iterative_scan" in s for s in calls_str))
        self.assertTrue(any("SET max_parallel_maintenance_workers" in s for s in calls_str))
        

    @patch('mem0.vector_stores.aurora.PSYCOPG_VERSION', 3)
    @patch('mem0.vector_stores.aurora.ConnectionPool')
    @patch.object(Aurora, '_get_cursor')
    def test_create_col_with_ivfflat_psycopg3(self, mock_get_cursor, mock_connection_pool):
        """Test collection creation with HNSW index."""
        # Set up mock pool and cursor
        mock_pool = MagicMock()
        mock_connection_pool.return_value = mock_pool
        ...
        # Verify ivfflat index creation query was executed
        ivf_calls = [call for call in self.mock_cursor.execute.call_args_list 
                     if "USING ivfflat" in str(call)]
        self.assertTrue(len(ivf_calls) > 0)
        self.assertEqual(aurora.collection_name, "test_collection")
        self.assertEqual(aurora.embedding_model_dims, 3)
        self.assertEqual(aurora.ivfflat_list, 100)

        calls_str = [str(c) for c in self.mock_cursor.execute.call_args_list]
        self.assertTrue(any("SET ivfflat.iterative_scan" in s for s in calls_str))
        self.assertTrue(any("SET max_parallel_maintenance_workers" in s for s in calls_str))
    ...
    def test_pool_cleanup_psycopg3(self):
        ...
        with patch('mem0.vector_stores.aurora.PSYCOPG_VERSION', 3), \
             patch('mem0.vector_stores.aurora.ConnectionPool') as mock_connection_pool:
    ...
    def test_pool_cleanup_psycopg2(self):
        ...
        with patch('mem0.vector_stores.aurora.PSYCOPG_VERSION', 2), \
             patch('mem0.vector_stores.aurora.ConnectionPool') as mock_connection_pool:
