...
class TestDocumentDB:
    ...
    @pytest.fixture
    def documentdb_instance_with_options(self, mock_mongo_client):
        mock_client, mock_db, mock_collection = mock_mongo_client
        with patch.object(DocumentDB, "create_col", return_value=mock_collection):
            return DocumentDB(
                db_name="test_db",
    ...
    def test_init(self, documentdb_instance):
        ...
        assert documentdb_instance.similarity == "cosine"
        assert documentdb_instance.num_candidates is None

    def test_init_with_options(self, documentdb_instance_with_options):
        assert documentdb_instance_with_options.similarity == "euclidean"
        assert documentdb_instance_with_options.num_candidates == 50
    ...
    def test_search(self, documentdb_instance):
        ...
        mock_results = [{"_id": "id1", "payload": {"text": "test1"}}, {"_id": "id2", "payload": {"text": "test2"}}]
    ...
    def test_search_with_custom_num_candidates(self, documentdb_instance_with_options):
        mock_results = [{"_id": "id1", "payload": {"text": "test1"}}]

        documentdb_instance_with_options.collection.list_indexes.return_value = [
            {"name": "test_collection_vector_index"}
        ]
        documentdb_instance_with_options.collection.aggregate.return_value = mock_results
        documentdb_instance_with_options.client.__getitem__.return_value.__getitem__.return_value = (
            documentdb_instance_with_options.collection
        )

        query_vector = [0.1, 0.2, 0.3]
        results = documentdb_instance_with_options.search("test query", query_vector, limit=2)

        assert len(results) == 1
        # Verify num_candidates is used from config
        assert documentdb_instance_with_options.num_candidates == 50
    ...
    def test_list(self, documentdb_instance):
        mock_docs = [{"_id": "id1", "payload": {"text": "test1"}}, {"_id": "id2", "payload": {"text": "test2"}}]
