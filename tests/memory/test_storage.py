...
class TestSQLiteManager:
    ...
    def test_get_last_messages_returns_newest_in_insertion_order(self, memory_manager):
        """get_last_messages(limit=N) must return the newest N in insertion order.

        A batched save stamps one created_at on every row, so ordering the inner LIMIT by
        the tied created_at keeps the oldest N instead of the newest N. Retrieving fewer
        than the saved count forces that selection, so this only holds with the rowid tiebreak.
        """
        messages = [{"role": "user", "content": f"msg{i}", "name": None} for i in range(8)]
        memory_manager.save_messages(messages, "scope-b")

        order = [m["content"] for m in memory_manager.get_last_messages("scope-b", limit=5)]

        assert order == ["msg3", "msg4", "msg5", "msg6", "msg7"]
