...
`pr-gate.yml` carries two jobs on two different events, each with an `if:` naming its own `github.event_name`. The `gate` job closes on `pull_request_target`. The `reopen` job runs on `issues: labeled`, and when the label is `accepted` it walks `closedByPullRequestsReferences` back to the pull requests that link that issue and reopens them. Without it the label a maintainer applies is inert: the gate only ever re-judges a PR on a PR event, so an author who did everything right waited for a human to reopen by hand. It reopens only pull requests carrying the `<!-- pr-gate -->` marker on a comment, so a PR a maintainer closed deliberately is never resurrected. Reopening runs through `GITHUB_TOKEN`, which by design raises no further workflow events, so the `gate` job cannot bounce the PR straight back out.

The gate does not listen for `edited`. That event fires on a title or description change, so with it the gate re-judged pull requests that had been open for days and closed them the moment their author touched the description, which is what actually closed #6948. The recovery path the gate's own comment describes ends in "reopen", and `reopened` covers that, so nothing is lost. The tradeoff is that an author who strips `Closes #<number>` back out after passing keeps an open PR, which a reviewer sees anyway.
