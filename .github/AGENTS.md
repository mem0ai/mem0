...
Vouch's only built-in exemptions are accounts ending in `[bot]` and repo collaborators with `write` or `admin`. **Organization membership alone is not an exemption**, which is why all mem0ai org members are listed explicitly and must stay listed. The PR gate is unaffected by this: `author_association` is `MEMBER` for every org member regardless of repository permission.
