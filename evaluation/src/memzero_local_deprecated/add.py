...
class MemoryADD:
    def __init__(self, data_path=None, batch_size=2, is_graph=False):
        self.mem0 = Memory()
        self.mem0.config.prompts.memory_creation = custom_instructions
    ...
    def add_memory(self, user_id, message, metadata, retries=3):
        for attempt in range(retries):
            try:
                _ = self.mem0.add(
                    data=message, user_id=user_id, metadata=metadata,
                )
                return
    ...
    def process_conversation(self, item, idx):
        ...
        self.mem0.delete_user_memories(user_id=speaker_a_user_id)
        self.mem0.delete_user_memories(user_id=speaker_b_user_id)
