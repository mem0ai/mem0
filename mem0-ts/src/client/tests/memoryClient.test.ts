...
describe('MemoryClient API', () => {
  ...
  it('should retrieve the specific memory by ID', async () => {
    ...
    expect(memory.metadata === null || typeof memory.metadata === 'object').toBe(true);

    // Should be an array of strings or null (categories)
    expect(Array.isArray(memory.categories) || memory.categories === null).toBe(true);
    ...
    expect(new Date(memory.created_at || '').toString()).not.toBe('Invalid Date');

    // Should be a valid date (updated_at)
    expect(new Date(memory.updated_at || '').toString()).not.toBe('Invalid Date');
  });
  ...
  it('should search and return results based on provided query and filters (API version 2)', async () => {
    ...
    const searchResultV2 = await client.search('What do you know about me?', searchOptionsV2);
  ...
  it('should search and return results based on provided query (API version 1)', async () => {
    const searchResultV1 = await client.search('What is my name?', { user_id: userId });
  ...
  it('should retrieve history of a specific memory and validate the fields', async () => {
    ...
    if (res22.length > 0) {
      ...
      expect(historyEntry.old_memory === null || typeof historyEntry.old_memory === 'string').toBe(
        true,
      );

      // Should be a string or null (new memory)
      expect(historyEntry.new_memory === null || typeof historyEntry.new_memory === 'string').toBe(
        true,
      );

      // Should be an array of strings or null (categories)
      expect(Array.isArray(historyEntry.categories) || historyEntry.categories === null).toBe(true);
      ...
      // Should be a valid date (created_at)
      expect(new Date(historyEntry.created_at).toString()).not.toBe('Invalid Date');

      // Should be a valid date (updated_at)
      expect(new Date(historyEntry.updated_at).toString()).not.toBe('Invalid Date');
      ...
      expect(Array.isArray(historyEntry.input) || historyEntry.input === null).toBe(true);
