...
function isValidEmbedding(value: unknown): value is EmbeddingResponse {
  ...
  return (
    ...
    obj.embeddings !== null &&
    Array.isArray((obj.embeddings as Record<string, unknown>).values)
  );
}
...
export class VertexAIEmbedder implements Embedder {
  ...
  constructor(config: VertexAIConfig) {
    ...
    this.projectId = config.googleProjectId || process.env.GCP_PROJECT_ID || "";
    ...
    const clientOptions: any = { apiEndpoint: endpoint };

    if (config.vertexCredentialsJson) {
      clientOptions.keyFilename = config.vertexCredentialsJson;
  ...
  async embed(
    ...
    const decoded = helpers.fromValue(response.predictions[0] as any);
    if (!isValidEmbedding(decoded)) {
      throw new Error("Failed to extract embedding values from response");
    }

    return decoded.embeddings.values as number[];
  }
  ...
  async embedBatch(
    ...
    for (let i = 0; i < texts.length; i += batchSize) {
      ...
      const instances = chunk.map(
        (text) => helpers.toValue(this.formatInstance(text)) as any,
      );
      const parameters = {
        taskType: embeddingType,
        outputDimensionality: this.embeddingDims,
      ...
      for (const prediction of response.predictions) {
        const decoded = helpers.fromValue(prediction as any);
        if (!isValidEmbedding(decoded)) {
          throw new Error(
            "Failed to extract embedding values from batch response",
          );
        }
        allEmbeddings.push(decoded.embeddings.values as number[]);
      }
    }
