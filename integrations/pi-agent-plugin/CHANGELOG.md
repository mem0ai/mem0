...
- **Relevance-filtered search** — `/mem0-search`, `/mem0-forget`, and `/mem0-pin` filter out weak semantic matches below a similarity threshold (`searchThreshold`, default `0.2`; configurable in `mem0-config.json`, shown in `/mem0-status`). mem0 ranks results by similarity with no relevance floor, so an unrelated query previously returned the closest (weak) memories — and `/mem0-forget` offered them for deletion. Filtering is done **client-side on the public score** (mem0's recommended hard floor; the API-side `threshold` is applied pre-decay and reshapes scores, so passing it over-filtered and hid real matches — it is no longer sent). Search also enables mem0's **reranking** (`rerank`), a cross-encoder pass that spreads scores apart (genuine matches high, noise low) so the floor can actually tell a real hit from the closest noise — plain cosine scores cluster too tightly (~0.2–0.3 for both real and irrelevant queries) to separate. `/mem0-search` now shows each result's relevance score, and when the floor hides matches it reports the closest hidden score and how to lower the threshold, so results never silently vanish.
