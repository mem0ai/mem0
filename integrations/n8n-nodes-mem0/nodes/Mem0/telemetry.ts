...
// nor break a workflow. Opt out with MEM0_TELEMETRY=false.
// Uses native fetch + crypto only — no runtime dependencies (verified-node rule).
...
export function captureEvent(
	...
	try {
		...
		const distinctId = apiKey ? createHash('md5').update(apiKey).digest('hex') : 'n8n-anon';
		const payload = {
		...
		/* swallow — telemetry must never break the workflow */
	}
}
