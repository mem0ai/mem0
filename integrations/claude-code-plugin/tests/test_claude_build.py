...
def test_native_claude_bundle_preserves_working_contract(tmp_path: Path) -> None:
    ...
    assert (root / ".claude-plugin" / "plugin.json").read_bytes() == (
        HOST / ".claude-plugin" / "plugin.json"
    ).read_bytes()
    assert (root / ".mcp.json").read_bytes() == (HOST / ".mcp.json").read_bytes()
    assert (root / "hooks" / "hooks.json").read_bytes() == (HOST / "hooks" / "hooks.json").read_bytes()
    assert (root / "agents" / "sidekick.md").read_bytes() == (HOST / "agents" / "sidekick.md").read_bytes()
    values = {
