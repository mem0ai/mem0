...
- `resolve_repo()` collapses every subdirectory to the git root: `app_id` is
  the repository slug, `project_id` is the shared namespace (the same slug for
  a repository with a remote, `local-<folder>-<path hash>` otherwise) and
  `directory` is the working directory relative to the root ("" at the root).
...
- `core/mcp_server.py` is the documented exception: an MCP server is a separate
  ...
  mid-session.
- Every flush sends a single unified add call carrying both `agent_id` (project)
  and `user_id` (personal) together with `app_id: directory_app_id(repo)`,
  `run_id: session_id`, `agent_custom_instructions`, `custom_instructions`,
  and `custom_categories`. The Platform classifies each extracted memory as
  project or personal internally. `metadata.author` is set to the `user_id`
  and `metadata.dirs` carries the directory chain for subtree filtering.
- `_search_filters(user, repo, scope)` is the only place a search filter is
  built and it is deterministic. `mine` is `{user_id}`; `repo` is
  `OR[{agent_id}, {user_id}]`; `dir` replaces the shared branch with
  `AND[{agent_id}, {app_id: directory}]` and collapses to `repo` at the root.
  `run_id` and `category` are nested with `AND` under that filter, never in
  place of it. No scope ever builds a `user_id: "*"` filter, and a flush with
  no session ID is refused (`{"status": "error", "reason": "no-session-id"}`).
- Never filter on `app_id: "*"`. The wildcard matches only non-null values.
  Exact `app_id` only: the Platform ignores `icontains` on `app_id`.
- `forget_remote_repo()` deletes only the caller's `user_id` in this `app_id`.
  Project memory is shared, so it is deleted only with
  `include_project_memory=True`, which adds `OR {agent_id: project_id}`.
