...
export default function MemoriesPage() {
  ...
  const fetchMemories = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const res = await api.get(MEMORY_ENDPOINTS.BASE, { params: { user_id: userId } });
        ...
        <Card className="border-memBorder-primary">
          <CardContent className="p-4 space-y-2">
            <div className="flex justify-between items-start">
              ...
              <button onClick={() => setSelectedMemory(null)} className="text-xs text-onSurface-default-tertiary hover:underline">Close</button>
            </div>
            ...
            <div className="flex gap-4 text-xs text-onSurface-default-tertiary">
              ...
              {selectedMemory.user_id && <span>User: {selectedMemory.user_id}</span>}
              {selectedMemory.agent_id && <span>Agent: {selectedMemory.agent_id}</span>}
            </div>
          </CardContent>
        </Card>
      )}
