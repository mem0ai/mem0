...
const classes = {
  ...
  tableHeaderCell: "w-[230px] h-[38px] p-2 align-middle bg-surface-default-fg-secondary text-onSurface-default-secondary",
  tableHeaderCheckCell: "w-[40px] h-[38px] p-2 align-middle bg-surface-default-fg-secondary",
  tableHeaderCheckWrap: "flex items-center gap-2",
  tableHeaderCheckBox: "box-border flex h-4 w-4 items-center gap-2.5 rounded-sm border border-memBorder-primary p-1",
  tableHeaderDivider: "w-px self-stretch border border-memBorder-primary shrink-0",
  tableRow: "h-[38px] border-t border-memBorder-primary bg-surface-default-primary hover:bg-surface-default-primary-hover",
  tableCell: "text-sm font-medium text-onSurface-default-primary px-4 py-2 justify-start align-middle font-[Fustat] leading-[140%] tracking-normal",
      ...
      <table className="table-fixed w-full">
        <colgroup>
          {columns.map((col, i) => {
            ...
            const pct = totalWeight > 0 ? (weight / totalWeight) * 100 : 100 / columns.length
          ...
          <tr className={classes.tableHeaderRow}>
            {columns.map((column, index) => {
              ...
              const relevantClasses = column.className
                ? column.className.split(' ').filter(c =>
                  c.startsWith('w-') ||
                  c.startsWith('min-w-') ||
                  c.startsWith('max-w-') ||
              ...
              const alignClass =
                column.align === 'center'
                  ...
                  : column.align === 'right'
                    ? 'text-right'
                    : ''
              const mergedRelevantClasses = `${relevantClasses} ${alignClass}`.trim()
              const hasCustomAlignment = mergedRelevantClasses.includes('text-center') || mergedRelevantClasses.includes('text-right')
              ...
              const flexAlignment = mergedRelevantClasses.includes('text-center')
                ? 'justify-center'
              ...
              return (
                <th key={index} className={headerCellClassName}>
                  <div className="flex h-full min-w-0 items-stretch justify-between">
                    <div className={`flex min-w-0 flex-1 items-center gap-2 overflow-hidden ${flexAlignment}`}>
        ...
        <tbody className="transition-all duration-300 ease-in-out">
          {data.map((row, rowIndex) => (
            <tr
              ...
              className={`${classes.tableRow} ${onRowClick ? "cursor-pointer" : ""
                } ${getRowClassName ? getRowClassName(row, rowIndex) ?? "" : ""} animate-fade-in`}
              onClick={onRowClick ? () => onRowClick(row, rowIndex) : undefined}
            >
              {columns.map((column, colIndex) => {
                const value = row[column.key]
                const baseCellClass = column.cellVariant === "flush" ? classes.tableCellFlush : classes.tableCell
